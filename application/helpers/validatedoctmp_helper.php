...
//Check รหัสสินค้าที่ใส่มา ต้องอยู่ภายใน AD ของตัวเอง
function FCNnDocTmpChkPDTCodeINADSelf(array $paParams = [],$paCondition){
    if(
        !isset($paParams['tUserSessionID']) || empty($paParams['tUserSessionID']) 
        || !isset($paParams['tFieldName']) || empty($paParams['tFieldName']) 
        || !isset($paParams['tTableName']) || empty($paParams['tTableName'])
    ...
    $tSQL = "UPDATE T SET 
                T.FTTmpStatus = '2', 
                T.FTTmpRemark = 'รหัสสินค้านี้พบอยู่ในร้านค้าอื่น' 
            FROM TCNTDocDTTmp T 
            INNER JOIN (
                SELECT 
                        PDT.FTSessionID ,
                        PDT.FTPdtCode ,
                        PDT.FTPunCode 
                FROM TCNTDocDTTmp PDT WITH(NOLOCK)
                INNER JOIN TCNMPdtSpcBch SPC ON PDT.FTPdtCode = SPC.FTPdtCode 
                WHERE 
                    PDT.FTXthDocKey = '$tTablename' 
                AND PDT.FTTmpStatus = 1
                AND PDT.FTSessionID = '$tUserSessionID'
                AND '$tAGNCode' <> ISNULL(SPC.FTAgnCode,'')
