var nStaChnBrowseType = $('#oetSeaStaBrowse').val();
var tCallChnBackOption = $('#oetSeaCallBackOption').val();
...
$('document').ready(function() {
    ...
    if (nStaChnBrowseType != 1) {
        JSvCallPageFashionSeason(1);
    } else {
        JSvCallPageFashionSeasonAdd();
    }
...
function JSxChanelNavDefult() {
    try {
        if (nStaChnBrowseType != 1 || nStaChnBrowseType == undefined) {
            ...
            $('#odvModalBody #odvSeaMainMenu').removeClass('main-menu');
 ...
 * Return Type : view
 */
function JSvCallPageFashionSeason(pnPage) {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "fashionseasonList",
            cache: false,
            timeout: 0,
            success: function(tResult) {
                $('#odvContentPageChanel').html(tResult);
                JSvFashionSeasonDataTable(pnPage);
            },
        ...
        console.log('JSvCallPageFashionSeason Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvFashionSeasonDataTable(pnPage) {
    try {
        ...
        $.ajax({
            ...
            success: function(tResult) {
                if (tResult != "") {
                    $('#ostDataFashionSeason').html(tResult);
                }
                JSxChanelNavDefult();
                JCNxLayoutControll();
        ...
        console.log('JSvFashionSeasonDataTable Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvCallPageFashionSeasonAdd() {
    try {
        JCNxOpenLoading();
        JStCMMGetPanalLangSystemHTML('', '');
        $.ajax({
            type: "POST",
            url: "fashionseasonPageAdd",
            cache: false,
            timeout: 0,
            success: function(tResult) {
                if (nStaChnBrowseType == 1) {
        ...
        console.log('JSvCallPageFashionSeasonAdd Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvCallPageFashionSeasonEdit(ptSeaCode, ptChnBchCode) {
    try {

        JCNxOpenLoading();
        JStCMMGetPanalLangSystemHTML('JSvCallPageFashionSeasonEdit', ptSeaCode);

        $.ajax({
            type: "POST",
            url: "fashionseasonPageEdit",
            data: { tSeaCode: ptSeaCode, tChnBchCode: ptChnBchCode },
            cache: false,
            timeout: 0,
            success: function(tResult) {
                if (tResult != '') {
                    $('#oliSeaTitleAdd').hide();
                    $('#oliSeaTitleEdit').show();
                    $('#odvBtnSeaInfo').hide();
                    $('#odvBtnAddEdit').show();
                    $('#odvContentPageChanel').html(tResult);
                    $('#oetSeaCode').addClass('xCNDisable');
        ...
        console.log('JSvCallPageFashionSeasonEdit Error: ', err);
    }
}
...
//Return Type : object
function JSnAddEditFashionSeason(ptRoute) {

    $('#ofmAddFashionSeason').validate().destroy();
    $('#ofmAddFashionSeason').validate({
        rules: {
            oetSeaCode: {
                "required": {
                    depends: function(oElement) {
                        if (ptRoute == "fashionseasonEventAdd") {
            ...
            oetWahBchNameCreated: {
                "required": {
                    depends: function(oElement) {
                        if ($('#oetSeaUsrLoginLevel') != "HQ" && $('#oetSeaUsrLoginLevel') != "AGN") {
                            return false;
                        } else {
                            return true;
                        }
        ...
        messages: {
            ...
            oetSeaName: {
                "required": $('#oetSeaName').attr('data-validate-required'),
            },
            oetChnAppName: {
        ...
        submitHandler: function(form) {
            $.ajax({
                type: "POST",
                url: ptRoute,
                data: $('#ofmAddFashionSeason').serialize(),
                cache: false,
                timeout: 0,
                success: function(tResult) {
                    ...
                    if (aReturn['nStaEvent'] == 1) {
                        if (aReturn['nStaCallBack'] == '1' || aReturn['nStaCallBack'] == null) {
                            JSvCallPageFashionSeasonEdit(aReturn['tCodeReturn'], aReturn['tCodeBchReturn']);
                        } else if (aReturn['nStaCallBack'] == '2') {
                            JSvCallPageFashionSeasonAdd();
                        } else if (aReturn['nStaCallBack'] == '3') {
                            JSvCallPageFashionSeason();
                        }
                    } else {
...
function JSaFashionSeasonDelete(pnPage, tCode, ptName) {
        ...
        $('#osmConfirm').on('click', function(evt) {
            $.ajax({
                ...
                success: function(tResult) {
                    ...
                    var aReturn = JSON.parse(tResult);
                    if (aReturn['nStaEvent'] == 1) {
                        $('#odvModalDelFashionSeason').modal('hide');
                        ...
                        setTimeout(function() {
                            if (aReturn["nNumRowChnLoc"] != 0) {
                                if (aReturn["nNumRowChnLoc"] > 10) {
                                    ...
                                    if (pnPage <= nNumPage) {
                                        JSvCallPageFashionSeason(pnPage);
                                    } else {
                                        JSvCallPageFashionSeason(nNumPage);
                                    }
                                } else {
                                    JSvCallPageFashionSeason(1);
                                }
 ...
 * Return Type : view
 */
function JSvFashionSeasonClickPage(ptPage) {
    try {
        var nPageCurrent = '';
        var nPageNew;
        switch (ptPage) {
        ...
        JSvFashionSeasonDataTable(nPageCurrent);
    } catch (err) {
        console.log('JSvFashionSeasonClickPage Error: ', err);
    }
}
 ...
 * Return Type : -
 */
function JSxFashionSeasonDeleteRowHead(poElement = null, poEvent = null) {
    try {
 ...
 * Return Type : -
 */
function JSxFashionSeasonDeleteRowEnd(poElement = null, poEvent = null) {
    try {
...
function JCNbChanelIsCreatePage() {
    try {
        const tChnCode = $('#oetSeaCode').data('is-created');
        var bStatus = false;
...
function JCNbChanelIsUpdatePage() {
    try {
        const tVCode = $('#oetSeaCode').data('is-created');
        let bStatus = false;
        if (tVCode != "") { // Have data
            bStatus = true;
        }
    ...
    if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
        ...
        for (var nI = 0; nI < ocbListItem.length; nI++) {
            if ($($(".ocbListItem").eq(nI)).prop('checked')) {
                aDataSeaCode.push($($(".ocbListItem").eq(nI)).parents('.xWChnItems').data('seacode'));

            }
        }
        let aDataWhere = {
            'paSeaCode': aDataSeaCode,
        };
        $.ajax({
            type: "POST",
            url: "fashionseasonDeleteMulti",
...
function JSvCallListSeasonChain(ptSeaCode,ptSeaChain, ptSeaLevel) {
    try {
        var base_url = $('#ohdbaseURL').val();
        // alert(base_url)
        var nCurrentPageTB = $('#nCurrentPageTB').val();
        JCNxOpenLoading();
        $.ajax({
            type: "POST",
            url: "fashionseasonDataTableChain",
            data: {
                tSeaChain: ptSeaChain,
                tSeaLevel: ptSeaLevel
            },
            cache: false,
            Timeout: 5000,
            success: function(oResult) {
                ...
                console.log(aReturn)
                if (aReturn['rtCode'] == 1) {
                    $.each(aReturn.raHDItems, function(i) {
                        if(aReturn.raHDItems[i]['rnStaChrl']>0){
                            var tButtonChril = '<button type="button" onclick="JSvCallListSeasonChain(\'' + aReturn.raHDItems[i]['FTSeaCode'] + '\',\'' + aReturn.raHDItems[i]['FTSeaChain'] + '\',\'' + aReturn.raHDItems[i]['FNSeaLevel'] + '\')"><i class="fa fa-angle-right" aria-hidden="true"></button></i> ';
                        }else{
                            var  tButtonChril = '';
                        }
                        // alert('dd' + aReturn.raHDItems[i]['FTSeaChain'])
                        var row = $('<tr class="text-center xCNTextDetail2 xWChnItems xWSeaTR' + aReturn.raHDItems[i]['FTSeaCode'] + ' xCNSublevel' + aReturn.raHDItems[i]['FNSeaLevel'] + '" data-key="" data-seacode="' + aReturn.raHDItems[i]['FTSeaCode'] + '"  data-lev="'+ aReturn.raHDItems[i]['FNSeaLevel'] +'" data-chain="'+ aReturn.raHDItems[i]['FTSeaChain'] +'"  >' +
                            '<td><label class="fancy-checkbox"><input id="ocbListItem" type="checkbox" class="ocbListItem" name="ocbListItem[]"></label></td>' +
                            '<td class="text-left">' + aReturn.raHDItems[i]['FTSeaCode'] + '</td>' +
                            '<td class="text-left"><span style="padding-right:' + aReturn.raHDItems[i]['FNSeaLevel'] + '0px;"></span>'+tButtonChril+aReturn.raHDItems[i]['FTSeaName'] + '</td>' +
                            '<td><img class="xCNIconTable" src="' + base_url + '/application/modules/common/assets/images/icons/delete.png"  title="ลบ" onclick="JSaFashionSeasonDelete(\''+nCurrentPageTB+'\',\''+ aReturn.raHDItems[i]['FTSeaCode'] +'\',\''+aReturn.raHDItems[i]['FTSeaName']+'\')"></td>' +
                            '<td><img class="xCNIconTable" src="' + base_url + '/application/modules/common/assets/images/icons/edit.png" onClick="JSvCallPageFashionSeasonEdit(\'' + aReturn.raHDItems[i]['FTSeaCode'] + '\')" title="แก้ไข"></td>' +
                            '</tr>');

                            $('#otbSeason .xWSeaTR' + ptSeaCode + '').after(row);
                    });
                       //level 0
                       $('#otbSeason .xWSeaTR' + ptSeaCode + '').find('button').attr('onclick', 'JSxRemoveLevelOne("' + ptSeaCode + '","' + ptSeaChain+ '","' + ptSeaLevel + '")')
                       $('#otbSeason .xWSeaTR' + ptSeaCode + '').find('i').attr('class', 'fa fa-angle-down')

                       // console.log('#otbSeason .xWSeaTR' + ptSeaChain + '').find('i').attr('class', 'fa-angle-down')
...
function JSxRemoveLevelOne(ptSeaCode, ptSeaChain, ptSeaLevel) {
     
    // $('#otbSeason .xCNSublevel' + ptSeaLevel + '').remove();

    $('.xWChnItems').each(function(oElement){
        var tChain = $(this).data('chain');
        // var nCHainLev = parseFloat($('.xWSeaTR'+ptSeaCode).data('level'));
        var nLevel = parseFloat($(this).data('lev'));
        var tCodePK = $(this).data('seacode');
        
        // alert(tChain+' = '+ptSeaCode);
        // alert(nLevel+' > '+ptSeaLevel);
        var nCount = tChain.indexOf(ptSeaCode);
      
            if(nCount!='-1' && nLevel>ptSeaLevel){
                $('.xWSeaTR'+tCodePK).remove();
    ...
    // $('#otbSeason .xCNSublevel' + ptSeaLevel + '').remove();
    $('#otbSeason .xWSeaTR' + ptSeaCode + '').find('button').attr('onclick', 'JSvCallListSeasonChain("' + ptSeaCode + '","'+ptSeaChain+'","' + ptSeaLevel + '")')
    $('#otbSeason .xWSeaTR' + ptSeaCode + '').find('i').attr('class', 'fa fa-angle-right')
