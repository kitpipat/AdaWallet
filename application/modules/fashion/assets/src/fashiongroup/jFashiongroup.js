var nStaChnBrowseType = $('#oetPgpStaBrowse').val();
var tCallChnBackOption = $('#oetPgpCallBackOption').val();
...
$('document').ready(function() {
    ...
    if (nStaChnBrowseType != 1) {
        JSvCallPageFashionGroup(1);
    } else {
        JSvCallPageFashionGroupAdd();
    }
 ...
 * Return Type : view
 */
function JSvCallPageFashionGroup(pnPage) {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "fashiongroupList",
            cache: false,
            timeout: 0,
            success: function(tResult) {
                $('#odvContentPageChanel').html(tResult);
                JSvFashionGroupDataTable(pnPage);
            },
        ...
        console.log('JSvCallPageFashionGroup Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvFashionGroupDataTable(pnPage) {
    try {
        ...
        $.ajax({
            ...
            success: function(tResult) {
                if (tResult != "") {
                    $('#ostDataFashionGroup').html(tResult);
                }
                JSxChanelNavDefult();
                JCNxLayoutControll();
        ...
        console.log('JSvFashionGroupDataTable Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvCallPageFashionGroupAdd() {
    try {
        JCNxOpenLoading();
        JStCMMGetPanalLangSystemHTML('', '');
        $.ajax({
            type: "POST",
            url: "fashiongroupPageAdd",
            cache: false,
            timeout: 0,
            success: function(tResult) {
                if (nStaChnBrowseType == 1) {
        ...
        console.log('JSvCallPageFashionGroupAdd Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvCallPageFashionGroupEdit(ptPgpCode, ptChnBchCode) {
    try {

        JCNxOpenLoading();
        JStCMMGetPanalLangSystemHTML('JSvCallPageFashionGroupEdit', ptPgpCode);

        $.ajax({
            type: "POST",
            url: "fashiongroupPageEdit",
            data: { tPgpCode: ptPgpCode, tChnBchCode: ptChnBchCode },
            cache: false,
            timeout: 0,
            success: function(tResult) {
                if (tResult != '') {
        ...
        console.log('JSvCallPageFashionGroupEdit Error: ', err);
    }
}
...
//Return Type : object
function JSnAddEditFashionGroup(ptRoute) {

    $('#ofmAddFashionGroup').validate().destroy();
    $('#ofmAddFashionGroup').validate({
        rules: {
            oetPgpCode: {
                "required": {
                    depends: function(oElement) {
                        if (ptRoute == "fashiongroupEventAdd") {
            ...
            oetWahBchNameCreated: {
                "required": {
                    depends: function(oElement) {
                        if ($('#oetPgpUsrLoginLevel') != "HQ" && $('#oetPgpUsrLoginLevel') != "AGN") {
                            return false;
                        } else {
                            return true;
                        }
        ...
        submitHandler: function(form) {
            $.ajax({
                type: "POST",
                url: ptRoute,
                data: $('#ofmAddFashionGroup').serialize(),
                cache: false,
                timeout: 0,
                success: function(tResult) {
                    ...
                    if (aReturn['nStaEvent'] == 1) {
                        if (aReturn['nStaCallBack'] == '1' || aReturn['nStaCallBack'] == null) {
                            JSvCallPageFashionGroupEdit(aReturn['tCodeReturn'], aReturn['tCodeBchReturn']);
                        } else if (aReturn['nStaCallBack'] == '2') {
                            JSvCallPageFashionGroupAdd();
                        } else if (aReturn['nStaCallBack'] == '3') {
                            JSvCallPageFashionGroup();
                        }
                    } else {
...
function JSaFashionGroupDelete(pnPage, tCode, ptName) {
        ...
        $('#osmConfirm').on('click', function(evt) {
            $.ajax({
                ...
                success: function(tResult) {
                    ...
                    var aReturn = JSON.parse(tResult);
                    if (aReturn['nStaEvent'] == 1) {
                        $('#odvModalDelFashionGroup').modal('hide');
                        ...
                        setTimeout(function() {
                            if (aReturn["nNumRowChnLoc"] != 0) {
                                if (aReturn["nNumRowChnLoc"] > 10) {
                                    ...
                                    if (pnPage <= nNumPage) {
                                        JSvCallPageFashionGroup(pnPage);
                                    } else {
                                        JSvCallPageFashionGroup(nNumPage);
                                    }
                                } else {
                                    JSvCallPageFashionGroup(1);
                                }
 ...
 * Return Type : view
 */
function JSvFashionGroupClickPage(ptPage) {
    try {
        var nPageCurrent = '';
        var nPageNew;
        switch (ptPage) {
        ...
        JSvFashionGroupDataTable(nPageCurrent);
    } catch (err) {
        console.log('JSvFashionGroupClickPage Error: ', err);
    }
}
 ...
 * Return Type : -
 */
function JSxFashionGroupDeleteRowHead(poElement = null, poEvent = null) {
    try {
 ...
 * Return Type : -
 */
function JSxFashionGroupDeleteRowEnd(poElement = null, poEvent = null) {
    try {
...
function JCNbChanelIsCreatePage() {
    try {
        const tChnCode = $('#oetPgpCode').data('is-created');
        var bStatus = false;
...
function JCNbChanelIsUpdatePage() {
    try {
        const tVCode = $('#oetPgpCode').data('is-created');
        let bStatus = false;
        if (tVCode != "") { // Have data
            bStatus = true;
        }
    ...
    if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
        ...
        for (var nI = 0; nI < ocbListItem.length; nI++) {
            if ($($(".ocbListItem").eq(nI)).prop('checked')) {
                aDataPgpCode.push($($(".ocbListItem").eq(nI)).parents('.xWChnItems').data('clscode'));

            }
        }
        let aDataWhere = {
            'paPgpCode': aDataPgpCode,
        };
        $.ajax({
            type: "POST",
            url: "fashiongroupDeleteMulti",
