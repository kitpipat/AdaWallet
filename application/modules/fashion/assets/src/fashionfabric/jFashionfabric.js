var nStaChnBrowseType = $('#oetFabStaBrowse').val();
var tCallChnBackOption = $('#oetFabCallBackOption').val();
...
$('document').ready(function() {
    ...
    if (nStaChnBrowseType != 1) {
        JSvCallPageFashionFabric(1);
    } else {
        JSvCallPageFashionFabricAdd();
    }
...
function JSxChanelNavDefult() {
    try {
        if (nStaChnBrowseType != 1 || nStaChnBrowseType == undefined) {
            ...
            $('#odvModalBody #odvFabMainMenu').removeClass('main-menu');
 ...
 * Return Type : view
 */
function JSvCallPageFashionFabric(pnPage) {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "fashionfabricList",
            cache: false,
            timeout: 0,
            success: function(tResult) {
                $('#odvContentPageChanel').html(tResult);
                JSvFashionFabricDataTable(pnPage);
            },
        ...
        console.log('JSvCallPageFashionFabric Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvFashionFabricDataTable(pnPage) {
    try {
        ...
        $.ajax({
            ...
            success: function(tResult) {
                if (tResult != "") {
                    $('#ostDataFashionFabric').html(tResult);
                }
                JSxChanelNavDefult();
                JCNxLayoutControll();
        ...
        console.log('JSvFashionFabricDataTable Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvCallPageFashionFabricAdd() {
    try {
        JCNxOpenLoading();
        JStCMMGetPanalLangSystemHTML('', '');
        $.ajax({
            type: "POST",
            url: "fashionfabricPageAdd",
            cache: false,
            timeout: 0,
            success: function(tResult) {
                if (nStaChnBrowseType == 1) {
        ...
        console.log('JSvCallPageFashionFabricAdd Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvCallPageFashionFabricEdit(ptFabCode, ptChnBchCode) {
    try {

        JCNxOpenLoading();
        JStCMMGetPanalLangSystemHTML('JSvCallPageFashionFabricEdit', ptFabCode);

        $.ajax({
            type: "POST",
            url: "fashionfabricPageEdit",
            data: { tFabCode: ptFabCode, tChnBchCode: ptChnBchCode },
            cache: false,
            timeout: 0,
            success: function(tResult) {
                if (tResult != '') {
                    $('#oliFabTitleAdd').hide();
                    $('#oliFabTitleEdit').show();
                    $('#odvBtnFabInfo').hide();
                    $('#odvBtnAddEdit').show();
                    $('#odvContentPageChanel').html(tResult);
                    $('#oetFabCode').addClass('xCNDisable');
        ...
        console.log('JSvCallPageFashionFabricEdit Error: ', err);
    }
}
...
//Return Type : object
function JSnAddEditFashionFabric(ptRoute) {

    $('#ofmAddFashionFabric').validate().destroy();
    $('#ofmAddFashionFabric').validate({
        rules: {
            oetFabCode: {
                "required": {
                    depends: function(oElement) {
                        if (ptRoute == "fashionfabricEventAdd") {
            ...
            oetWahBchNameCreated: {
                "required": {
                    depends: function(oElement) {
                        if ($('#oetFabUsrLoginLevel') != "HQ" && $('#oetFabUsrLoginLevel') != "AGN") {
                            return false;
                        } else {
                            return true;
                        }
        ...
        messages: {
            ...
            oetFabName: {
                "required": $('#oetFabName').attr('data-validate-required'),
            },
            oetChnAppName: {
        ...
        submitHandler: function(form) {
            $.ajax({
                type: "POST",
                url: ptRoute,
                data: $('#ofmAddFashionFabric').serialize(),
                cache: false,
                timeout: 0,
                success: function(tResult) {
                    ...
                    if (aReturn['nStaEvent'] == 1) {
                        if (aReturn['nStaCallBack'] == '1' || aReturn['nStaCallBack'] == null) {
                            JSvCallPageFashionFabricEdit(aReturn['tCodeReturn'], aReturn['tCodeBchReturn']);
                        } else if (aReturn['nStaCallBack'] == '2') {
                            JSvCallPageFashionFabricAdd();
                        } else if (aReturn['nStaCallBack'] == '3') {
                            JSvCallPageFashionFabric();
                        }
                    } else {
...
function JSaFashionFabricDelete(pnPage, tCode, ptName) {
        ...
        $('#osmConfirm').on('click', function(evt) {
            $.ajax({
                ...
                success: function(tResult) {
                    ...
                    var aReturn = JSON.parse(tResult);
                    if (aReturn['nStaEvent'] == 1) {
                        $('#odvModalDelFashionFabric').modal('hide');
                        ...
                        setTimeout(function() {
                            if (aReturn["nNumRowChnLoc"] != 0) {
                                if (aReturn["nNumRowChnLoc"] > 10) {
                                    ...
                                    if (pnPage <= nNumPage) {
                                        JSvCallPageFashionFabric(pnPage);
                                    } else {
                                        JSvCallPageFashionFabric(nNumPage);
                                    }
                                } else {
                                    JSvCallPageFashionFabric(1);
                                }
 ...
 * Return Type : view
 */
function JSvFashionFabricClickPage(ptPage) {
    try {
        var nPageCurrent = '';
        var nPageNew;
        switch (ptPage) {
        ...
        JSvFashionFabricDataTable(nPageCurrent);
    } catch (err) {
        console.log('JSvFashionFabricClickPage Error: ', err);
    }
}
 ...
 * Return Type : -
 */
function JSxFashionFabricDeleteRowHead(poElement = null, poEvent = null) {
    try {
 ...
 * Return Type : -
 */
function JSxFashionFabricDeleteRowEnd(poElement = null, poEvent = null) {
    try {
...
function JCNbChanelIsCreatePage() {
    try {
        const tChnCode = $('#oetFabCode').data('is-created');
        var bStatus = false;
...
function JCNbChanelIsUpdatePage() {
    try {
        const tVCode = $('#oetFabCode').data('is-created');
        let bStatus = false;
        if (tVCode != "") { // Have data
            bStatus = true;
        }
    ...
    if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
        ...
        for (var nI = 0; nI < ocbListItem.length; nI++) {
            if ($($(".ocbListItem").eq(nI)).prop('checked')) {
                aDataFabCode.push($($(".ocbListItem").eq(nI)).parents('.xWChnItems').data('fabcode'));

            }
        }
        let aDataWhere = {
            'paFabCode': aDataFabCode,
        };
        $.ajax({
            type: "POST",
            url: "fashionfabricDeleteMulti",
