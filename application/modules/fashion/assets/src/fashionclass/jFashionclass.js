var nStaChnBrowseType = $('#oetClsStaBrowse').val();
var tCallChnBackOption = $('#oetClsCallBackOption').val();
...
$('document').ready(function() {
    ...
    if (nStaChnBrowseType != 1) {
        JSvCallPageFashionClass(1);
    } else {
        JSvCallPageFashionClassAdd();
    }
...
function JSxChanelNavDefult() {
    try {
        if (nStaChnBrowseType != 1 || nStaChnBrowseType == undefined) {
            ...
            $('#odvModalBody #odvClsMainMenu').removeClass('main-menu');
 ...
 * Return Type : view
 */
function JSvCallPageFashionClass(pnPage) {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "fashionclassList",
            cache: false,
            timeout: 0,
            success: function(tResult) {
                $('#odvContentPageChanel').html(tResult);
                JSvFashionClassDataTable(pnPage);
            },
        ...
        console.log('JSvCallPageFashionClass Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvFashionClassDataTable(pnPage) {
    try {
        ...
        $.ajax({
            ...
            success: function(tResult) {
                if (tResult != "") {
                    $('#ostDataFashionClass').html(tResult);
                }
                JSxChanelNavDefult();
                JCNxLayoutControll();
        ...
        console.log('JSvFashionClassDataTable Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvCallPageFashionClassAdd() {
    try {
        JCNxOpenLoading();
        JStCMMGetPanalLangSystemHTML('', '');
        $.ajax({
            type: "POST",
            url: "fashionclassPageAdd",
            cache: false,
            timeout: 0,
            success: function(tResult) {
                if (nStaChnBrowseType == 1) {
        ...
        console.log('JSvCallPageFashionClassAdd Error: ', err);
    }
}
 ...
 * Return Type : view
 */
function JSvCallPageFashionClassEdit(ptClsCode, ptChnBchCode) {
    try {

        JCNxOpenLoading();
        JStCMMGetPanalLangSystemHTML('JSvCallPageFashionClassEdit', ptClsCode);

        $.ajax({
            type: "POST",
            url: "fashionclassPageEdit",
            data: { tClsCode: ptClsCode, tChnBchCode: ptChnBchCode },
            cache: false,
            timeout: 0,
            success: function(tResult) {
                if (tResult != '') {
                    $('#oliClsTitleAdd').hide();
                    $('#oliClsTitleEdit').show();
                    $('#odvBtnClsInfo').hide();
                    $('#odvBtnAddEdit').show();
                    $('#odvContentPageChanel').html(tResult);
                    $('#oetClsCode').addClass('xCNDisable');
        ...
        console.log('JSvCallPageFashionClassEdit Error: ', err);
    }
}
...
//Return Type : object
function JSnAddEditFashionClass(ptRoute) {

    $('#ofmAddFashionClass').validate().destroy();
    $('#ofmAddFashionClass').validate({
        rules: {
            oetClsCode: {
                "required": {
                    depends: function(oElement) {
                        if (ptRoute == "fashionclassEventAdd") {
            ...
            oetWahBchNameCreated: {
                "required": {
                    depends: function(oElement) {
                        if ($('#oetClsUsrLoginLevel') != "HQ" && $('#oetClsUsrLoginLevel') != "AGN") {
                            return false;
                        } else {
                            return true;
                        }
        ...
        messages: {
            ...
            oetClsName: {
                "required": $('#oetClsName').attr('data-validate-required'),
            },
            oetChnAppName: {
        ...
        submitHandler: function(form) {
            $.ajax({
                type: "POST",
                url: ptRoute,
                data: $('#ofmAddFashionClass').serialize(),
                cache: false,
                timeout: 0,
                success: function(tResult) {
                    ...
                    if (aReturn['nStaEvent'] == 1) {
                        if (aReturn['nStaCallBack'] == '1' || aReturn['nStaCallBack'] == null) {
                            JSvCallPageFashionClassEdit(aReturn['tCodeReturn'], aReturn['tCodeBchReturn']);
                        } else if (aReturn['nStaCallBack'] == '2') {
                            JSvCallPageFashionClassAdd();
                        } else if (aReturn['nStaCallBack'] == '3') {
                            JSvCallPageFashionClass();
                        }
                    } else {
...
function JSaFashionClassDelete(pnPage, tCode, ptName) {
        ...
        $('#osmConfirm').on('click', function(evt) {
            $.ajax({
                ...
                success: function(tResult) {
                    ...
                    var aReturn = JSON.parse(tResult);
                    if (aReturn['nStaEvent'] == 1) {
                        $('#odvModalDelFashionClass').modal('hide');
                        ...
                        setTimeout(function() {
                            if (aReturn["nNumRowChnLoc"] != 0) {
                                if (aReturn["nNumRowChnLoc"] > 10) {
                                    ...
                                    if (pnPage <= nNumPage) {
                                        JSvCallPageFashionClass(pnPage);
                                    } else {
                                        JSvCallPageFashionClass(nNumPage);
                                    }
                                } else {
                                    JSvCallPageFashionClass(1);
                                }
 ...
 * Return Type : view
 */
function JSvFashionClassClickPage(ptPage) {
    try {
        var nPageCurrent = '';
        var nPageNew;
        switch (ptPage) {
        ...
        JSvFashionClassDataTable(nPageCurrent);
    } catch (err) {
        console.log('JSvFashionClassClickPage Error: ', err);
    }
}
 ...
 * Return Type : -
 */
function JSxFashionClassDeleteRowHead(poElement = null, poEvent = null) {
    try {
 ...
 * Return Type : -
 */
function JSxFashionClassDeleteRowEnd(poElement = null, poEvent = null) {
    try {
...
function JCNbChanelIsCreatePage() {
    try {
        const tChnCode = $('#oetClsCode').data('is-created');
        var bStatus = false;
...
function JCNbChanelIsUpdatePage() {
    try {
        const tVCode = $('#oetClsCode').data('is-created');
        let bStatus = false;
        if (tVCode != "") { // Have data
            bStatus = true;
        }
    ...
    if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
        ...
        for (var nI = 0; nI < ocbListItem.length; nI++) {
            if ($($(".ocbListItem").eq(nI)).prop('checked')) {
                aDataClsCode.push($($(".ocbListItem").eq(nI)).parents('.xWChnItems').data('clscode'));

            }
        }
        let aDataWhere = {
            'paClsCode': aDataClsCode,
        };
        $.ajax({
            type: "POST",
            url: "fashionclassDeleteMulti",
