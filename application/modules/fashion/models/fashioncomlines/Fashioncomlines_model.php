...
class Fashioncomlines_model extends CI_Model
{
    ...
    public function FSaMFSCSearchByID($paData)
    {
        ...
        $tCmlCode = $paData['FTCmlCode'];
        $nLngID = $paData['FNLngID'];

        //  query
        $tHDSQL = "SELECT
FSC.FTCmlCode   AS rtCmlCode,
FSC.FTAgnCode AS rtAgnCode,
FSCL.FTCmlName AS rtCmlName,
FSCL.FTCmlRmk AS rtCmlRmk,
      AGNL.FTAgnName As   rtAgnName,
      FDCreateOn
      FROM [TFHMPdtF5ComLines] FSC WITH(NOLOCK)
  LEFT JOIN TFHMPdtF5ComLines_L   FSCL WITH(NOLOCK) ON FSC.FTCmlCode = FSCL.FTCmlCode AND FSCL.FNLngID = $nLngID
  LEFT JOIN   TCNMAgency_L AGNL WITH(NOLOCK) ON FSC.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID = $nLngID
  WHERE 1=1
  AND FSC.FTCmlCode = '$tCmlCode'";
    ...
    public function FSaMFSCList($ptAPIReq, $ptMethodReq, $paData)
    {
        ...
        if ($tSearchList != '') {
            $tWhereCondition .= " AND (FSC.FTCmlCode LIKE '%$tSearchList%' ";
            $tWhereCondition .= " OR FSCL.FTCmlName LIKE '%$tSearchList%') ";
        }

        $tSQL1 = " SELECT c.* FROM( SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtCmlCode DESC) AS rtRowID,* FROM ( ";
     ...
     * Parameters : $ptCmlCode
     * Creator : 26/04/2021 Worakorn
     * Last Modified : -
     * Return : data
     * Return Type : object is has result, boolean is no result
     */
    public function FSoMFSCCheckDuplicate($ptCmlCode)
    {
        $tSQL = "SELECT COUNT(FTCmlCode) AS counts
                 FROM TFHMPdtF5ComLines
                 WHERE FTCmlCode = '$ptCmlCode'";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            return $oQuery->result();
        } else {
    ...
    public function FSaMFSCAddUpdateHD($paData)
    {
        try {
            if ($paData['tTypeInsertUpdate'] == 'Update') {
                ...
                $this->db->where('FTCmlCode', $paData['FTCmlCode']);
                $this->db->update('TFHMPdtF5ComLines');

                $this->db->set('FNLngID', $paData['FNLngID']);
                $this->db->set('FTCmlName', $paData['FTCmlName']);
                $this->db->set('FTCmlRmk', $paData['FTCmlRmk']);
                $this->db->where('FTCmlCode', $paData['FTCmlCode']);
                $this->db->update('TFHMPdtF5ComLines_L');
                ...
                // Insert
                $this->db->insert('TFHMPdtF5ComLines', array(
                ...
                $this->db->insert('TFHMPdtF5ComLines_L', array(
                    'FTCmlCode'   => $paData['FTCmlCode'],
                    'FNLngID' => $paData['FNLngID'],
                    'FTCmlName'   => $paData['FTCmlName'],
                    'FTCmlRmk'   => $paData['FTCmlRmk'],
                ));
    ...
    public function FSnMFSCDelHD($paData)
    {
        ...
        $this->db->where_in('FTCmlCode', $paData['FTCmlCode']);
        $this->db->delete('TFHMPdtF5ComLines');


        $this->db->where_in('FTCmlCode', $paData['FTCmlCode']);
        $this->db->delete('TFHMPdtF5ComLines_L');
    ...
    {
        $tSQL = "SELECT COUNT(*) AS FNAllNumRow FROM TFHMPdtF5ComLines";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
    ...
    public function  FSaMFSCDeleteMultiple($paDataDelete)
    {
        ...
        $this->db->where_in('FTCmlCode', $paDataDelete['FTCmlCode']);
        $this->db->delete('TFHMPdtF5ComLines');
