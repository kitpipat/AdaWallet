...
class cProducttransferwahousevd extends MX_Controller {
	...
	public function __construct() {
            ...
            $this->load->model('document/producttransferwahousevd/mProducttransferwahousevd');
    }
    ...
    public function index($nBrowseType,$tBrowseOption){
            ...
            $aData['aAlwEvent']       = FCNaHCheckAlwFunc('TWXVD/0/0'); //Controle Event
            $aData['vBtnSave']          = FCNaHBtnSaveActiveHTML('TWXVD/0/0'); //Load Html ของปุ่ม Save ที่เก็บ Session ปัจจุบัน
            //Get Option Show Decimal
            $aData['nOptDecimalShow']   = FCNxHGetOptionDecimalShow(); 
            $aData['nOptDecimalSave']   = FCNxHGetOptionDecimalSave(); 
            
            
            $this->load->view('document/producttransferwahousevd/wProducttransferwahousevd',$aData);
    ...
    public function FSvCTFWGetHDDisTableData(){
        ...
        $this->load->view('document/producttransferwahousevd/advancetable/wProducttransferwahouseHDDisTableDatavd',$aData);
    ...
    public function FSvCTFWGetDTDisTableData(){
        ...
        $this->load->view('document/producttransferwahousevd/advancetable/wProducttransferwahouseDTDisTableDatavd',$aData);
    ...
    public function FSaCTFWEditEvent(){
        try{
            ...
            $aDataMaster = array(
                ...
                'FTXthPosFrm'            => $this->input->post('oetPosCodeStart'),
                'FTXthPosTo'             => $this->input->post('oetPosCodeEnd'),
            ...
            $aStaEventOrdHDRef          = $this->mProducttransferwahousevd->FSaMTFWAddUpdateHDRef($aDataHDSpl);
            ...
            $aStaEventTmpToDT           = $this->FSaMTFWAddTmpToDT($aDataMaster['FTXthDocNo'],$this->input->post('oetBchCode'));
    ...
    //Function : Add DT
    public function FSaMTFWAddTmpToDT($ptXthDocNo,$tBchCode){
    ...
    */
    ...
    public function FSxCTFWAddPage(){
        ...
        $this->load->view('document/producttransferwahousevd/wProducttransferwahouseAddvd',$aDataAdd);
    ...
    public function FSvCTFWEditPage(){
       ...
       //Lang ภาษา
        ...
        //Get Data
        $aResult        = $this->mProducttransferwahousevd->FSaMTFWGetHD($aDataWhere);       // Data TVDTPdtTwxHD
        $aDataHDRef     = $this->mProducttransferwahousevd->FSaMTFWGetHDRef($aDataWhere);    // Data TVDTPdtTwxHDRef
        // $aDataDT        = $this->mProducttransferwahousevd->FSaMTFWGetDT($aDataWhere);       // Data TVDTPdtTwxDT
        // // $aStaIns        = $this->mProducttransferwahousevd->FSaMTFWInsertDTToTemp($aDataDT,$aDataWhere);    // Insert Data DocTemp
        ...
        $this->load->view('document/producttransferwahousevd/wProducttransferwahouseAddvd',$aDataEdit);
    ...
    public function FSxCTFWDataTable(){
        ...
        $this->load->view('document/producttransferwahousevd/wProducttransferwahouseDataTablevd',$aGenTable);
    }
    ...
    public function FSxCTWXVDPdtDtLoadToTem(){
        ...
        $tDocno = "";
        $tStaApv = $this->input->post("tStaApvPanelPdt");
        $tStaPrcStk = $this->input->post("tStaPrcStkPanelPdt");
        ...
        $aInfoWhere = array(
            "tBchCode"=>$this->input->post("tBchCodePanelPdt"),
            "tShpCode"=>$this->input->post("tShpCodePanelPdt"),
            "tWahCode"=>$this->input->post("tWahCodePanelPdt"),
            "nLangID"=>$nLangEdit,
            "tDocNo"=>$tDocno
        );
        $aDataInfor = "";
        if (!($tDocno != '' && ($tStaApv == 1 && $tStaPrcStk == 1))) {
            ...
            $aDataInfor = $this->mProducttransferwahousevd->FSaMTWXVDGetPdtLayoutToTemFromDT($aInfoWhere);
        ...
        if($aData){
            if($tPanelPdtType==1){ // pos to shop
                if (!($tDocno != '' && ($tStaApv == 1 && $tStaPrcStk == 1))) {
                    for($nI=0;$nI<FCNnHSizeOf($aData);$nI++){
                        $aData[$nI]["maxTransfer"] = $aData[$nI]["FCStkQty"];
                    }
                }
            }else if($tPanelPdtType==2){ // shop to pos
                if (!($tDocno != '' && ($tStaApv == 1 && $tStaPrcStk == 1))) {
                    $aPosKey = array();
                    $aShopKey = array();
                    for($nI=0;$nI<FCNnHSizeOf($aData);$nI++){
                        $nRealReceiveEnd = $aData[$nI]["FCLayColQtyMax"] - $aData[$nI]["FCStkQty"];
                        array_push($aPosKey,array(
                            "row"=>$aData[$nI]["FNLayRow"],
                            "col"=>$aData[$nI]["FNLayCol"],
                            "pdt"=>$aData[$nI]["FTPdtCode"],
                            "stk"=>$nRealReceiveEnd
                        ));
                        $aInfoWhere = array(
                            "tBchCode"=>$this->input->post("tBchCode"),
                        ...
                        $nStkPdtStart = $this->mProducttransferwahousevd->FSnMGetStkPdt($aInfoWhere);
                        array_push($aShopKey,array(
                            "pdt"=>$aData[$nI]["FTPdtCode"],
                            "stk"=>$nStkPdtStart
                        ));
                    }
                    
                    for($nI=0;$nI<FCNnHSizeOf($aShopKey);$nI++){
                        if($aShopKey[$nI]["pdt"]!="NULL"){
                            for($nJ=0;$nJ<FCNnHSizeOf($aShopKey);$nJ++){
                                if($aShopKey[$nI]["pdt"]==$aShopKey[$nJ]["pdt"] &&
                                    $nI!=$nJ){
                                    $aShopKey[$nJ]["pdt"] = "NULL";
                                }
                            }
                        }
                    }
                    $aBufferShopKey = array();
                    for($nI=0;$nI<FCNnHSizeOf($aShopKey);$nI++){
                        if($aShopKey[$nI]["pdt"]!="NULL"){
                            array_push($aBufferShopKey,array(
                                "pdt"=>$aShopKey[$nI]["pdt"],
                                "stk"=>$aShopKey[$nI]["stk"]
                            ));
                        }
                    }
                    $aShopKey = $aBufferShopKey;
                    for($nI=0;$nI<FCNnHSizeOf($aPosKey);$nI++){
                        for($nJ=0;$nJ<FCNnHSizeOf($aShopKey);$nJ++){
                            if($aPosKey[$nI]["pdt"]==$aShopKey[$nJ]["pdt"]){
                                if($aPosKey[$nI]["stk"]<=$aShopKey[$nJ]["stk"]){
                                    $aPosKey[$nI]["maxTransfer"] = $aPosKey[$nI]["stk"];
                                    $aShopKey[$nJ]["stk"] = $aShopKey[$nJ]["stk"]-$aPosKey[$nI]["stk"];
                                }else if($aShopKey[$nJ]["stk"]<=$aPosKey[$nI]["stk"]){
                                    $aPosKey[$nI]["maxTransfer"] = $aShopKey[$nJ]["stk"];
                                    $aShopKey[$nJ]["stk"] = 0;
                                }
                                break;
                            }
                    ...
                    for($nI=0;$nI<FCNnHSizeOf($aData);$nI++){
                        for($nJ=0;$nJ<FCNnHSizeOf($aPosKey);$nJ++){
                            if($aData[$nI]["FNLayRow"]==$aPosKey[$nJ]["row"] &&
                                $aData[$nI]["FNLayCol"]==$aPosKey[$nJ]["col"]){
                                $aData[$nI]["maxTransfer"] = $aPosKey[$nJ]["maxTransfer"];
                                break; 
                            }
                        }
                    }
                }
            }else if($tPanelPdtType==3){ // pos to pos
                if (!($tDocno != '' && ($tStaApv == 1 && $tStaPrcStk == 1))) {
                    for($nI=0;$nI<FCNnHSizeOf($aData);$nI++){
                        $nRealReceiveEnd = $aData[$nI]["FCLayColQtyMax"] - $aData[$nI]["FCStkQty"];
                        $aInfoWhere = array(
                            "tBchCode"=>$this->input->post("tBchCode"),
                            "tSchCodeStart"=>$this->input->post("tSchCodeStart"),
                            "tWahCodeStart"=>$this->input->post("tWahCodeStart"),
                            "aData"=>$aData[$nI],
                            "nLangID"=>$nLangEdit
                        );
                        $nRealTransferStartInfor = $this->mProducttransferwahousevd->FSaMTWXVDGetRealTransferPosStart($aInfoWhere);
                        $nRealTransferStart = $nRealTransferStartInfor["FCStkQty"];
                        if($nRealTransferStart<=$nRealReceiveEnd){
                            $aData[$nI]["maxTransfer"] = $nRealTransferStart;
                        }else if($nRealReceiveEnd<=$nRealTransferStart){
                            $aData[$nI]["maxTransfer"] = $nRealReceiveEnd;
                        }
                    }
                }
            ...
            $aDataUserInput = false;
            if (!($tDocno != '' && ($tStaApv == 1 && $tStaPrcStk == 1))) {
                if($this->input->post("nStatusSetDTForEdit")==1){
                
                    
                    if($tRoute!="TWXVDEventAdd"){
                        $aDataUserInput = $this->mProducttransferwahousevd->FSxMGetDataFromDtToTmp(array(
                            "tBchCode"=>$this->input->post("tBchCode"),
                            "tXthDocNo"=>$this->input->post("tXthDocNo")
                        ));
                        //print_r($aDataUserInput);
                    }
                }
            }
            // $nCheckInsertPdt = 0;
            // for($nI=0;$nI<FCNnHSizeOf($aData);$nI++){
            //     if($aData[$nI]["maxTransfer"]==0){
            //         $nCheckInsertPdt++;
            //     }
            // }
            
            ////////////////////////////////////////
            // เก็บข้อมูลลง tem จากตัวแปร $aData
            ////////////////////////////////////////
            //if($nCheckInsertPdt!=FCNnHSizeOf($aData)){

            
                for($nI=0;$nI<FCNnHSizeOf($aData);$nI++){
                    $tFCUserInPutTransferForTWXVD = $aData[$nI]["maxTransfer"];
                    if($aDataUserInput){
                        for($nJ=0;$nJ<FCNnHSizeOf($aDataUserInput);$nJ++){
                            if($aData[$nI]["FNLayRow"]==$aDataUserInput[$nJ]["FNLayRow"] &&
                                $aData[$nI]["FNLayCol"]==$aDataUserInput[$nJ]["FNLayCol"]){
                                if($aData[$nI]["maxTransfer"]>=$aDataUserInput[$nJ]["FCXtdQty"]){
                                    $tFCUserInPutTransferForTWXVD = $aDataUserInput[$nJ]["FCXtdQty"];
                                }
                    ...
                    $aInforInsert = array(
                        ...
                        "FTXthDocNo"=>$tDocno,
                        "FNXtdSeqNo"=>($nI+1),
                        "FNLayRowForTWXVD"=>$aData[$nI]["FNLayRow"],
                        "FNLayColForTWXVD"=>$aData[$nI]["FNLayCol"],
                        "FCLayColQtyMaxForTWXVD"=>$aData[$nI]["FCLayColQtyMax"],
                        "FTPdtCode"=>$aData[$nI]["FTPdtCode"],
                        "FCStkQty"=>$aData[$nI]["FCStkQty"],
                        "FCMaxTransferForTWXVD"=>$aData[$nI]["maxTransfer"],
                        "FTXthDocKey"=>"TVDTPdtTwxHD",
                        "FTXtdPdtName"=>$aData[$nI]["FTPdtName"],
                        "FCUserInPutTransferForTWXVD"=>$tFCUserInPutTransferForTWXVD
                    );
    ...
    public function FSvCTFWPdtAdvTblLoadData(){
        ...
        $aInfor = array(
            ...
            "tPanelPdtType"=>$tPanelPdtType,
            "tPanelPdtStaShow"=>$tPanelPdtStaShow,
            "nPage"           => $nPage
        );
        $this->load->view('document/producttransferwahousevd/advancetable/wNewProducttransferwahousePdtAdvTableDatavd',$aInfor);
    ...
    */
    public function FSvCTFWVatLoadData(){
        ...
        $this->load->view('document/producttransferwahousevd/advancetable/wProducttransferwahouseVatTableDatavd',$aData);
    ...
    */
    ...
    public function FSvCTFWAdvTblShowColList(){
        ...
        $this->load->view('document/producttransferwahousevd/advancetable/wPurchaseTableShowColListvd',$aData);
    ...
    public function FSxCTFWFormSearchList(){
        ...
        $this->load->view('document/producttransferwahousevd/wProducttransferwahouseFormSearchListvd',$aDataMaster);
    }
