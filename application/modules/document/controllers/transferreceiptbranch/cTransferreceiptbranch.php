...
class cTransferreceiptbranch extends MX_Controller{
    ...
    public function FSoCTBIEventAddPdtIntoDocDTTemp() {
        try {
            ...
            $tTBIPdtData         = $this->input->post('aTBIPdtData');
            $nTBIptionAddPdt    = $this->input->post('nTBIptionAddPdt');
    ...
    public function FSoCTBIEventEdit(){
        try {
            ...
            $aCalDTTempParams = [
                'tDocNo'        => '',
                'tBchCode'      => $aDataDocument['oetTBIBchCode'],
                'tSessionID'    => $tTBISessionID,
                'tDocKey'       => 'TCNTPdtTbiHD'
            ];
    ...
    public function FSoCTBIEventApproved(){
        ...
        $aCheckRecive = $this->mTransferreceiptbranch->FSaMTBICheckRefDocRecive($aDataUpdate); 

            if($aCheckRecive['rtCode']=='1'){
                ...
                $aStaApv = $this->mTransferreceiptbranch->FSvMTBIApprove($aDataUpdate); 
                    ...
                    $aReturn = array(
                        'nStaEvent'    => '900',
                        'tStaMessg'    => 'เอกสารจ่ายโอนนี้ถูกอ้างอิงไปแล้ว'
                    );
                    echo json_encode($aReturn);
            }
    ...
    public function FSoCTBIEventGetPdtIntDTBch(){
        try{
            ...
            $tTBIDocNo =  $this->input->post('tTBIDocNo');
            $tTBIBchCodeTo =  $this->input->post('tTBIBchCodeTo'); //สาขาปลายทาง
            $tTBIBchCode =  $this->input->post('tTBIBchCode'); //สาขาที่สร้าง
           ...
           $this->mTransferreceiptbranch->FSoMTBIEventGetPdtIntDTFhnBchToTemp($aDataTBIGetPdtIntBch);//ดึงข้อมูลสินค้าแฟชั่นลง Temp
        ...
        //เพิ่มสินค้าลงตาราง Tmp
        public function FSoCTBIEventAddPdtIntoDTFhnTemp(){
            try {
                $tTWOUserLevel       = $this->session->userdata('tSesUsrLevel');
                $tTBIDocNo           = $this->input->post('tTBIDocNo');
                $tTBIBCHCode         = $this->input->post('tTBIBCH');
                $tTBIPdtDataFhn      = $this->input->post('tTBIPdtDataFhn');
                $aTBIPdtData         = JSON_decode($tTBIPdtDataFhn);
                $tTBIVATInOrEx       = 1;
                $tTypeInsPDT         = $this->input->post('tTBIType');
                ...
                $this->db->trans_begin();
                if($aTBIPdtData->tType=='confirm'){
                    // $aDataWhere['tPdtCode'] = $aTBIPdtData->aResult[0]->tPDTCode;
                    // FCNxClearDTFhnTmp($aDataWhere);
                    // ทำทีรายการ ตามรายการสินค้าที่เพิ่มเข้ามา
                    $nPdtParentQty = 0;
                    for ($nI = 0; $nI < FCNnHSizeOf($aTBIPdtData->aResult); $nI++) {
    
                        $aItem          = $aTBIPdtData->aResult[$nI];
                        $tTBIPdtCode    = $aItem->tPDTCode;
                        $tTBItRefCode   = $aItem->tRefCode;
                        $tTBItBarCode   = $aItem->tBarCode;
                        $tTBItPunCode   = $aItem->tPunCode;
                        $nTBInQty       = $aItem->nQty;
    
                        $nPdtParentQty  = $nPdtParentQty + $nTBInQty;
    
                        $aDataWhere['tPdtCode'] = $tTBIPdtCode;
                        $aDataWhere['tBarCode'] = $tTBItBarCode;
                        $aDataWhere['tPunCode'] = $tTBItPunCode;
                        
                        
                        if($nEvent==1){
                            $nTBISeqNo = FCNnGetMaxSeqDTFhnTmp($aDataWhere);
                        }else{
                            $nDTSeq         = $aItem->nDTSeq;
                            $nTBISeqNo      =  $nDTSeq;
                        }
                    ...
                    $tTBIPdtCode = $aTBIPdtData->aResult->tPDTCode;
                    $aDataPdtParams = array(
                        'tDocNo'            => $tTBIDocNo,
                        'tBchCode'          => $tTBIBCHCode,
                        'tPdtCode'          => $tTBIPdtCode,
