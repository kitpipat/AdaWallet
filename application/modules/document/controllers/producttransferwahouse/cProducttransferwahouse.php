...
class cProducttransferwahouse extends MX_Controller
{
    ...
    public function FSaCTFWEditEvent()
    {
        try {
            ...
            $aDataMaster = array(
                ...
                'FNXthStaDocAct'        => (int)$this->input->post('ocbXthStaDocAct'), //สถานะ ประมวลผลสต๊อก ว่าง หรือ Null:ยังไม่ทำ, 1:ทำแล้ว
    ...
    */
    ...
    public function FSoCTFWEventAddPdtIntoDTFhnTemp(){
        try {
            $tTFWUserLevel       = $this->session->userdata('tSesUsrLevel');
            $tTFWDocNo           = $this->input->post('tTFWDocNo');
            $tTFWBchCode         = $this->input->post('tTFWBCH');
            $tTFWPdtData         = $this->input->post('tTFWPdtDataFhn');
            $aTFWPdtData         = JSON_decode($tTFWPdtData);
            $tTFWVATInOrEx       = 1;
            $tTypeInsPDT         = $this->input->post('tTFWType');
            $nEvent              = $this->input->post('nEvent');
            $tOptionAddPdt       = $this->input->post('tOptionAddPdt');

            $aDataWhere = array(
                'tBchCode'  => $tTFWBchCode,
                'tDocNo'    => $tTFWDocNo,
                'tDocKey'   => 'TCNTPdtTwxHD',
            );
            $this->db->trans_begin();
            if($aTFWPdtData->tType=='confirm'){
                // $aDataWhere['tPdtCode'] = $aTFWPdtData->aResult[0]->tPDTCode;
                // FCNxClearDTFhnTmp($aDataWhere);
                // ทำทีรายการ ตามรายการสินค้าที่เพิ่มเข้ามา
                $nPdtParentQty = 0;
                for ($nI = 0; $nI < FCNnHSizeOf($aTFWPdtData->aResult); $nI++) {

                    $aItem          = $aTFWPdtData->aResult[$nI];
                    $tTFWPdtCode    = $aItem->tPDTCode;
                    $tTFWtRefCode   = $aItem->tRefCode;
                    $tTFWtBarCode   = $aItem->tBarCode;
                    $tTFWtPunCode   = $aItem->tPunCode;

                    $nTFWnQty       = $aItem->nQty;
                    $nPdtParentQty  = $nPdtParentQty + $nTFWnQty;
                    
                    $aDataWhere['tPdtCode'] = $tTFWPdtCode;
                    $aDataWhere['tBarCode'] = $tTFWtBarCode;
                    $aDataWhere['tPunCode'] = $tTFWtPunCode;

                    if($nEvent==1){
                        $nTFWSeqNo = FCNnGetMaxSeqDTFhnTmp($aDataWhere);
                    }else{
                        $nDTSeq         = $aItem->nDTSeq;
                        $nTFWSeqNo      =  $nDTSeq;
                    }
                    ...
                    $aDataPdtParams = array(
                        'tDocNo'            => $tTFWDocNo,
                        'tBchCode'          => $tTFWBchCode,
                        'tPdtCode'          => $tTFWPdtCode,
                        'tRefCode'          => $tTFWtRefCode,
                        'nMaxSeqNo'         => $nTFWSeqNo,
                        'nQty'              => $nTFWnQty,
                ...
                $tTFWPdtCode = $aTFWPdtData->aResult->tPDTCode;
                $aDataPdtParams = array(
                    'tDocNo'            => $tTFWDocNo,
                    'tBchCode'          => $tTFWBchCode,
                    'tPdtCode'          => $tTFWPdtCode,
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $aCalcDTParams = [
                    'tDataDocEvnCall'   => '1',
                    'tDataVatInOrEx'    => $tTFWVATInOrEx,
                    'tDataDocNo'        => $tTFWDocNo,
                    'tDataDocKey'       => 'TCNTPdtTwxHD',
                    'tDataSeqNo'        => ''
                ];
                $tStaCalcuRate = FCNbHCallCalcDocDTTemp($aCalcDTParams);
                if ($tStaCalcuRate === TRUE) {
                    // Prorate HD
                    FCNaHCalculateProrate('TCNTPdtTwxHD', $tTFWDocNo);
