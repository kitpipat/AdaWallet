...
class cRentalproductpriceadjustmentlocker extends MX_Controller {
	...
	public function __construct() {
            ...
            $this->load->model('document/rentalproductpriceadjustmentlocker/mRentalproductpriceadjustmentlocker');
    }
    ...
    public function index($nBrowseType,$tBrowseOption){
		...
		$aData['aAlwEvent']         = FCNaHCheckAlwFunc('ADJPL/0/0'); //Controle Event
        $aData['vBtnSave']          = FCNaHBtnSaveActiveHTML('ADJPL/0/0'); //Load Html ของปุ่ม Save ที่เก็บ Session ปัจจุบัน
        //Get Option Show Decimal
        $aData['nOptDecimalShow']   = FCNxHGetOptionDecimalShow(); 
        $aData['nOptDecimalSave']   = FCNxHGetOptionDecimalSave(); 
        $this->load->view('document/rentalproductpriceadjustmentlocker/wRentalproductpriceadjustmentlocker',$aData);
    ...
    public function FSvCTFWGetShipAdd(){
        ...
        $aDataShipAdd = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetAddress($tBchCode,$tXthShipAdd,$nLangEdit); 
    ...
    public function FSvCTFWGetHDDisTableData(){
        ...
        $this->load->view('document/rentalproductpriceadjustmentlocker/advancetable/wRentalproductpriceadjustmentlockerHDDisTableData',$aData);
    ...
    public function FSvCTFWGetDTDisTableData(){
        ...
        $this->load->view('document/rentalproductpriceadjustmentlocker/advancetable/wRentalproductpriceadjustmentlockerDTDisTableData',$aData);
    ...
    public function FSaCTFWEditEvent(){
        try{
            if($this->session->userdata('tSesUsrLevel')=="HQ"){
                $tBchCodeUsr = FCNtGetBchInComp();
            }else{
                $tBchCodeUsr = $this->session->userdata('tSesUsrBchCodeOld');
            }

            //ตรวจสอบ DocType 12/11/2019 Saharat(GolF)
            $CheckDate = $this->input->post('ohdCheckDateEnd');
            if($CheckDate == 1){
                $dDocDateStart  =  $this->input->post('oetXthDocDateStart');
                $dDocDateEnd    =  date('Y-m-d', strtotime('+5 year'));
            }else{
                $dDocDateStart  = $this->input->post('oetXthDocDateStart');
                $dDocDateEnd    = $this->input->post('oetXthDocDateEnd');
            }

            $aDataMaster = array(
                'FTBchCode'         => $tBchCodeUsr,  
                'FTXphDocNo'        => $this->input->post('oetXthDocNo'),
                'FTXphStaDoc'       => '1',       //Napat(Jame) 10/09/2019
                'FNXphStaDocAct'    => '1',    //Napat(Jame) 10/09/2019
                'FTXphDocType'      => $this->input->post('ocmPriceType'),
                'FDXphDocDate'      => $this->input->post('oetXthDocDate'),
                'FTXphDocTime'      => $this->input->post('oetXthDocTime'),
                'FTXphRefInt'       => $this->input->post('oetXthRefInt'),
                'FDXphRefIntDate'   => $this->input->post('oetXthRefIntDate') != '' ? $this->input->post('oetXthRefIntDate') : NULL, 
                'FDXphDStart'       => $dDocDateStart,
                'FTXphTStart'       => $this->input->post('oetXthDocTimeStart'),
                'FDXphDStop'        => $dDocDateEnd,
                'FTXphTStop'        => $this->input->post('oetXthDocTimeEnd'),
                'FTXphPriType'      => '4',
                // 'FNXphStaDocAct'=>$this->input->post('ocbXthStaDocAct'),
                'FTUsrCode'         => $this->input->post('oetUsrCode'),
                'FTMerCode'         => $this->input->post('oetMchCode'),
                'FTXphBchTo'        => $this->input->post('oetBchCode'),
                'FTXphRmk'          => $this->input->post('otaTfwRmk'),
            ...
            $aDataWhere = array(
                'FTBchCode'         => $tBchCodeUsr,
                'FTBchCodeForADJPL' => $this->input->post('oetBchCode'),
                'FTMerCodeForADJPL' => $this->input->post('oetMchCode'),
                'FTShpCodeForADJPL' => $this->input->post('oetShpCodeStart'),
            ...
            $this->db->trans_begin();
            $aStaSdtOrdHD               = $this->mRentalproductpriceadjustmentlocker->FSaMTFWAddUpdateHD($aDataMaster);
            $aStaEventDocNoInDocTemp    = $this->mRentalproductpriceadjustmentlocker->FSaMTFWAddUpdateDocNoInDocTemp($aDataWhere);    //Update DocNo ในตาราง Doctemp
            $aStaEventTmpToDT           =  $this->mRentalproductpriceadjustmentlocker->FSaMTFWInsertTmpToDT($aDataWhere);
    ...
    public function FSaMTFWSumDTIntoHD($ptXthDocNo){
        ...
        $aDataDT    =   $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetDT($aData); //*ลบ Data เก่าออก*/
        ...
        $aDataDT    =   $this->mRentalproductpriceadjustmentlocker->FSaMTFWUpdateHDFCXthTotal($aDataUpdHD,$tXthDocNo); //*ลบ Data เก่าออก*/
    ...
    public function FSvCTFWEditPdtIntoTableDT(){
        ...
        //Edit Inline
        $aResUpdDTTmpInline     = $this->mRentalproductpriceadjustmentlocker->FSnMTFWUpdateInlineDTTemp($aDataUpdateDT,$aDataWhere);
    ...
    public function FSnTFWUpdateHD(){
        ...
        //Get ค่า VATRate
        $cVatRate = $this->mRentalproductpriceadjustmentlocker->FScMTFWGetVatRateFromDoc($tXthDocNo);

        //Get ค่า VAT InOrEx
        $tXthVATInOrEx = $this->mRentalproductpriceadjustmentlocker->FCNxTFWGetvatInOrEx($tXthDocNo);

        //get จาก DT
        //ยอดรวมก่อนลด
        $cXthTotal          = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFCXthTotal($tXthDocNo); 
        //ยอดรวมมีภาษีห้ามลด
        $cXthVatNoDisChg    = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFCXthVatNoDisChg($tXthDocNo); 
        // echo "ยอดรวมมีภาษีห้ามลด :".$cXthVatNoDisChg."<br>";
        //ยอดรวมไม่มีภาษีห้ามลด
        $cXthNoVatNoDisChg  = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFCXthNoVatNoDisChg($tXthDocNo); 
        // echo "ยอดรวมไม่มีภาษีห้ามลด :".$cXthNoVatNoDisChg."<br>";
        //ยอดมีภาษีลดได้ 
        $cXthVatDisChgAvi   = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFCXthVatDisChgAvi($tXthDocNo); 
        // echo "ยอดมีภาษีลดได้ :".$cXthVatDisChgAvi."<br>";
        //ยอดไม่มีภาษีลดได้ 
        $cXthNoVatDisChgAvi = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFCXthNoVatDisChgAvi($tXthDocNo); 
        // echo "ยอดไม่มีภาษีลดได้ :".$cXthNoVatDisChgAvi."<br>";
    
        //ข้อมูลการ ลด ชาร์จ
        $aXthDisChgTxt = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFTXthDisChgTxt($tXthDocNo); 
        $tDisChgTxt = '';
        if($aXthDisChgTxt != 0){
        ...
        //get จาก HDDis
        //มูลค่ารวมส่วนลด
        $cXthDis            = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFCXthDis($tXthDocNo); 
        //มูลค่ารวมส่วนชาร์จ
        $cXthChg            = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFCXthChg($tXthDocNo); 

        //ยอดรวมมีภาษีหลังลด FCXthVatDisChgAvi-SUM(HDis.FCXddDisVat-HDis.FCXddChgVat)
        //Get SUM(HDis.FCXddDisVat-HDis.FCXddChgVat)
        $cXthDisRes    = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetSUMFCXddDisVatMinusFCXddChgVat($tXthDocNo);
        $cXthVatAfDisChg    = $cXthVatDisChgAvi-$cXthDisRes;
        // echo "ยอดรวมมีภาษีหลังลด :".$cXthVatAfDisChg."<br>";

        //ยอดรวมไม่มีภาษีหลังลด
        $cXthNoVatAfDisChg  = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFCXthNoVatAfDisChg($tXthDocNo);
        // echo $cXthNoVatAfDisChg."<br>";

        //ยอดมัดจำ 
        $cFCXthRefAEAmt  = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetFCXthRefAEAmt($tXthDocNo);
        ...
        if($oetXthWpTax != ''){
            ...
            $cXthWpTax = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDFCXthWpTax($tXthDocNo);
        }
        ...
        $aStaUpdOrdHD = $this->mRentalproductpriceadjustmentlocker->FSaMTFWUpdateOrdHD($Data,$DataWhere); /*ลงตาราง TAPTOrdHD */
    ...
    public function FSvCTFWApprove(){
        ...
        $aStaApv = $this->mRentalproductpriceadjustmentlocker->FSvMTFWApprove($aDataUpdate); 
    ...
    public function FSvCTFWCancel(){
        ...
        $aStaApv = $this->mRentalproductpriceadjustmentlocker->FSvMTFWCancel($aDataUpdate); 
    ...
    public function FSaMTFWAddDT(){
        ...
        $aStaEventDelDT =  $this->mRentalproductpriceadjustmentlocker->FSnMPMTDelPcoDT($this->input->post('oetSearchAll')); //*ลบ Data เก่าออก*/
        ...
        if($nNum != 0){
            foreach ($aDataFile['DTData'] as $key => $value) {
                ...
                $aStaEventOrdHDSpl  = $this->mRentalproductpriceadjustmentlocker->FSaMTFWAddUpdateOrdDT($value); /*ลงตาราง TAPTOrdDT */
    ...
    public function FSaMTFWAddDTDis(){
        ...
        $aStaEventDelDTDis =  $this->mRentalproductpriceadjustmentlocker->FSnMPMTDelPcoDTDis($tXthDocNo); //*ลบ Data เก่าออก*/
        ...
        if($nNum != 0){
            ...
            foreach ($aDataFile['DTData'] as $key => $valueDT) {
                ...
                foreach($valueDT['DTDiscount'] as $keyDis => $valueDTDis) {
                    ...
                    $aStaEventOrdDTDis  = $this->mRentalproductpriceadjustmentlocker->FSaMTFWAddUpdateOrdDTDis($valueDTDis); /*ลงตาราง TAPTOrdDTDis */
                ...
                $aStaUpdDT  = $this->mRentalproductpriceadjustmentlocker->FSaMTFWUpdateOrdDT($aDTData,$aDTDataWhere); /*Update TAPTOrdDT ใหม่ */
    ...
    public function FSaMTFWAddHDDis(){
        ...
        $aStaEventDelHDDis =  $this->mRentalproductpriceadjustmentlocker->FSnMPMTDelPcoHDDis($tXthDocNo); //* ลบ Data เก่าออก */
        ...
        if($nNum > 0){
            ...
            foreach ($aDataFile['HDData'] as $key => $value) {
                ...
                $aStaEventOrdHDDis  = $this->mRentalproductpriceadjustmentlocker->FSaMTFWAddUpdateHDDis($value); /*ลงตาราง TAPTOrdDT*/
    ...
    public function FSvCTFWAddPdtIntoTableDT(){
        ...
        $nCounts    = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetCountDTTemp($aDataWhere);
        ...
        for($nI=0;$nI<FCNnHSizeOf($aPdtData);$nI++){
            ...
            $aDataPdtMaster     =   $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetDataPdt($aDataPdtWhere); // Data Master Pdt
            
            


            
            $nStaInsPdtToTmp    =   $this->mRentalproductpriceadjustmentlocker->FSaMTFWInsertPDTToTemp($aDataPdtMaster,$aDataPdtWhere);
    ...
    */
    public function FSnCTFWCalulateDTTemp($ptXthDocNo,$ptXthVATInOrEx){
            ...
            //Get DT Tmp
            $aDataDTTmp = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetDTTemp($aDataWhere);
            ...
            if($aDataDTTmp['rtCode'] == 1){
                ...
                $aResUpd = $this->mRentalproductpriceadjustmentlocker->FSnMWTOUpdateDTTemp($aDataDTTmp,$aDataWhere);
    ...
    public function FSaCTFWDeleteEvent(){
        ...
        $aResDel    = $this->mRentalproductpriceadjustmentlocker->FSnMTFWDel($aDataMaster);
        $aReturn    = array(
    ...
    public function FSvCTFWRemovePdtInDTTmp(){
        ...
        $aResDel = $this->mRentalproductpriceadjustmentlocker->FSnMTFWDelDTTmp($aDataWhere);
    ...
    public function FSxCTFWAddPage(){
        ...
        $this->mRentalproductpriceadjustmentlocker->FSxMDeleteDoctemForNewEvent($aInfoWhere);
        
        

        //$this->mRentalproductpriceadjustmentlocker->FSxMClearPdtInTmp();
        //Get Option Show Decimal  
        $nOptDecimalShow = FCNxHGetOptionDecimalShow(); 
        ...
        $aDataUserGroup = $this->mRentalproductpriceadjustmentlocker->FStTFWGetShpCodeForUsrLogin($aDataShp); // Get ข้อมูลสาขา และร้านค้าของ User ที่ login
        if($aDataUserGroup==''){
            $tBchCode   = '';
            $tBchName   = '';
            $tMchCode   = '';
        ...
        $this->load->view('document/rentalproductpriceadjustmentlocker/wRentalproductpriceadjustmentlockerAdd',$aDataAdd);

    }

    public function FSbCheckHaveProductForTransfer(){
        $nNumPdt = $this->mRentalproductpriceadjustmentlocker->FSnMTFWCheckPdtTempForTransfer($this->input->post());
        if($nNumPdt>0){
    ...
    public function FSxCADJPLCheckDateTime(){
        $aEVNListDateTime = $this->mRentalproductpriceadjustmentlocker->FSxMADJPLCheckDateTime($this->input->post("tDocno"),$this->input->post("tPriceType"));
        if($aEVNListDateTime){
            
            $bCheckDateNoAllCompare = true;
            $tOetEvnTStartVal = $this->input->post("tXthDocTimeStart");
            $tOetEvnTFinishVal = $this->input->post("tXthDocTimeEnd");
            $tOetEvnDStartVal = $this->input->post("tXthDocDateStart");
            $tOetEvnDFinishVal = $this->input->post("tXthDocDateEnd");
            if($tOetEvnDStartVal!=$tOetEvnDFinishVal){
                $bCheckDateSecCompare = true;
                for($nJ = 0;$nJ<FCNnHSizeOf($aEVNListDateTime);$nJ++){
                    $tCompareDStartSend = $aEVNListDateTime[$nJ]["FDXphDStart"];
                    $tCompareDFinishSend = $aEVNListDateTime[$nJ]["FDXphDStop"];
                ...
                for($nJ = 0;$nJ<FCNnHSizeOf($aEVNListDateTime);$nJ++){
                    $tCompareTStartSend = $aEVNListDateTime[$nJ]["FTXphTStart"];
                    $tCompareTFinishSend = $aEVNListDateTime[$nJ]["FTXphTStop"];
            ...
            echo json_encode($bCheckDateNoAllCompare);
    ...
    public function FSbCheckHaveProductInDT(){
        ...
        $tBchCode = $this->input->post("tBchCode");
        $nNumPdt = $this->mRentalproductpriceadjustmentlocker->FSnMTFWCheckHaveProductInDT($tDocNo,$tBchCode);
        if($nNumPdt>0){
            echo json_encode(true);
        }else{
            echo json_encode(false);
    ...
    public function FSaCTFWAddEvent(){
        try{
            ...
            $aDataMaster = array(
                ...
                'FTXphTStop'        =>  $this->input->post('oetXthDocTimeEnd'),
                'FTXphPriType'      =>  '4',
                'FNXphStaDocAct'    =>  '1', //$this->input->post('ocbXthStaDocAct') Napat(Jame) 10/09/2019
                'FTUsrCode'         =>  $this->input->post('oetUsrCode'),
                'FTMerCode'         =>  $this->input->post('oetMchCode'),
                'FTXphBchTo'        =>  $this->input->post('oetBchCode'),
                'FTXphRmk'          =>  $this->input->post('otaTfwRmk'),
            ...
            if($aDataMaster['tIsAutoGenCode'] == '1'){ // Check Auto Gen Reason Code?
                ...
                // $aGenCode = FCNaHGenCodeV5('TCNTPdtAdjPriHD','9');
    ...
    public function FSvCTFWEditPage(){
        $aInfoWhere = array(
            "tXthDocNo"=>$tDocno = $this->input->post("ptXthDocNo"),
            "FTXthDocKey"=>"TCNTPdtAdjPriHD"
        );
        $this->mRentalproductpriceadjustmentlocker->FSxMDeleteDoctemForNewEvent($aInfoWhere);
       ...
       //Lang ภาษา
        ...
        $aAlwEvent = FCNaHCheckAlwFunc('ADJPL/0/0'); //Control Event
        //Get Option Show Decimal
        $nOptDecimalShow = FCNxHGetOptionDecimalShow(); 
        ...
        //Get Data
        $aResult        = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHD($aDataWhere);       // Data TVDTPdtTwxHD
        //$aDataHDRef     = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetHDRef($aDataWhere);    // Data TVDTPdtTwxHDRef
        // $aDataDT        = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetDT($aDataWhere);       // Data TVDTPdtTwxDT
        ...
        $aData = $this->mRentalproductpriceadjustmentlocker->FSaMTWXVDGetPdtLayoutToTem($aInfoWhere);
        if($aData){
            $aDataDT = $this->mRentalproductpriceadjustmentlocker->FSaMADJPGetDtLocker($aInfoWhere);
            
            for($nI=0;$nI<FCNnHSizeOf($aData);$nI++){
                $tRthCodeForADJPL = "";
                for($nJ=0;$nJ<FCNnHSizeOf($aDataDT);$nJ++){
                    if($aData[$nI]["FTPzeCode"]==$aDataDT[$nJ]["FTPzeCode"]){
                        $tRthCodeForADJPL = $aDataDT[$nJ]["FTRthCode"]; 
                        break;
                    }
                }
                ...
                $this->mRentalproductpriceadjustmentlocker->FSxMUpdateDocTmpPdtFromDT(array(
                    "tBchCode"=>$tBchCodeUsr,
                    "tFTBchCodeForADJPL"=>$aInfoWhere["tBchCode"],
                    "tMerCodeForADJPL"=>$aInfoWhere["tMerCode"],
                    "tShpCodeForADJPL"=>$aInfoWhere["tShpCode"],
                    "tXthDocNo"=>$aResult["raItems"]["FTXphDocNo"],
                    "tXtdSeqNo"=>($nI+1),
                    "tPzeCodeForADJPL"=>$aData[$nI]["FTPzeCode"],
                    "tSizNameForADJPL"=>$aData[$nI]["FTSizName"],
                    "tRthCodeForADJPL"=>$tRthCodeForADJPL,
                    "tPdtCode"=>$aData[$nI]["FTPdtCode"],
                    "tPunCode"=>$aData[$nI]["FTPunCode"]
                ));
        ...
        //$this->mRentalproductpriceadjustmentlocker->FSaMTFWInsertDTToTemp($aDataDT,$aDataWhere);    // Insert Data DocTemp
        ...
        $this->load->view('document/rentalproductpriceadjustmentlocker/wRentalproductpriceadjustmentlockerAdd',$aDataEdit);
    ...
    public function FSxCTFWDataTable(){
        ...
        //Controle Event
        $aAlwEvent          = FCNaHCheckAlwFunc('ADJPL/0/0'); 
        ...
        $aResList   = $this->mRentalproductpriceadjustmentlocker->FSaMTFWList($aData);
        $aGenTable  = array(
            'aAlwEvent'     => $aAlwEvent,
            'aDataList'     => $aResList,
            'nPage'         => $nPage,
        ...
        $this->load->view('document/rentalproductpriceadjustmentlocker/wRentalproductpriceadjustmentlockerDataTable',$aGenTable);
    }
    ...
    public function FSxCTWXVDPdtDtLoadToTem(){
        ...
        if($aData){
            for($nI=0;$nI<FCNnHSizeOf($aData);$nI++){
                $this->mRentalproductpriceadjustmentlocker->FSxMUpdateDocTmpPdtFromDT(array(
                    "tBchCode"=>$tBchCodeUsr,
                    "tFTBchCodeForADJPL"=>$this->input->post("tBchCode"),
                    "tMerCodeForADJPL"=>$this->input->post("tMerCode"),
                    "tShpCodeForADJPL"=>$this->input->post("tShpCode"),
                    "tXthDocNo"=>$tDocno,
                    "tXtdSeqNo"=>($nI+1),
                    "tPzeCodeForADJPL"=>$aData[$nI]["FTPzeCode"],
                    "tSizNameForADJPL"=>$aData[$nI]["FTSizName"],
    ...
    public function FSvCTFWPdtAdvTblLoadData(){
        ...
        $aDataInfor = $this->mRentalproductpriceadjustmentlocker->FSaMTWXVDGetPdtLayoutFRomTem($aInfoWhere);
        $aData = $aDataInfor["aDetail"];
        $tPanelPdtType=$this->input->post("tPanelPdtType");
        ...
        $this->load->view('document/rentalproductpriceadjustmentlocker/advancetable/wRentalproductpriceadjustmentlockerPdtAdvTableData',$aInfor);
        
    }

    public function FSxCADJPLPdtGetRateInfor(){
        //Lang ภาษา
        $nLangResort    = $this->session->userdata("tLangID");
        $nLangEdit      = $this->session->userdata("tLangEdit");
        // $aLangHave      = FCNaHGetAllLangByTable('TSysPmt_L');
        ...
        //     }
        // }
        $aDataInfor = $this->mRentalproductpriceadjustmentlocker->FSxMADJPLPdtGetRateInfor($nLangEdit);
        echo json_encode($aDataInfor);
    }

    public function FSxCADJPLPdtGetRateDTInfor(){
        $aDataInfor = $this->mRentalproductpriceadjustmentlocker->FSxMADJPLPdtGetRateDTInfor($this->input->post("tRateCode"));
        echo json_encode($aDataInfor);
    }

    public function FSxCADJPLPdtSaveRateDTInTmp(){
        $this->mRentalproductpriceadjustmentlocker->FSxMADJPLPdtSaveRateDTInTmp($this->input->post());
    }
    ...
    */
    public function FSvCTFWVatLoadData(){
        ...
        $aDataVatDT     = $this->mRentalproductpriceadjustmentlocker->FSaMTFWGetVatDTTemp($aDataWhere);

        $aData['nOptDecimalShow']   = $nOptDecimalShow;
        $aData['aDataVatDT']        = $aDataVatDT;
        $aData['tXthVATInOrEx']     = $tXthVATInOrEx;

        $this->load->view('document/rentalproductpriceadjustmentlocker/advancetable/wRentalproductpriceadjustmentlockerVatTableData',$aData);
    ...
    */
    public function FSxCTWXVDPdtUpdateTem(){
        $aDataWhere = array(
            ...
            "FNLayRowForTWXVD"=>$this->input->post("FNLayRow"),
            "FNLayColForTWXVD"=>$this->input->post("FNLayCol"),
            "FCUserInPutTransferForTWXVD"=>$this->input->post("FCUserInPutTransfer"),
            "FTXthDocKey"=>"TVDTPdtTwxHD",
            "FTSessionID"=>$this->session->userdata('tSesSessionID')
        );
        $this->mRentalproductpriceadjustmentlocker->FSxMUpdateTmpMaxTransfer($aDataWhere);
    ...
    */
    ...
    public function FSvCTFWAdvTblShowColList(){
        ...
        $this->load->view('document/rentalproductpriceadjustmentlocker/advancetable/wPurchaseTableShowColList',$aData);
    ...
    public function FSxCTFWFormSearchList(){
        ...
        $this->load->view('document/rentalproductpriceadjustmentlocker/wRentalproductpriceadjustmentlockerFormSearchList',$aDataMaster);
    }
    ...
    public function FSvCTFWPdtMultiDeleteEvent(){
        ...
        if($nCount > 1){
            ...
            for($i=0;$i<$nCount;$i++){
                ...
                $aResDel = $this->mRentalproductpriceadjustmentlocker->FSaMTFWPdtTmpMultiDel($aDataMaster);
            }
    ...
    public function FSxCTWXCheckViaCodeForApv(){
        $tDocNo = $this->input->post('tXthDocNo');
        $aResult = $this->mRentalproductpriceadjustmentlocker->FSxMTWXCheckViaCodeForApv($tDocNo);
        if($aResult){
