...
class cTransferBchOut extends MX_Controller
{
    ...
    public function FSaCTransferBchOutAddEvent()
    {
        try {
            ...
            $tDocNo = $this->input->post('oetTransferBchOutDocNo');
            $tBchCode = $this->input->post('oetTransferBchOutBchCode');
            $aCompanyInfo  = FCNaGetCompanyForDocument();
            $aEndOfBillParams = [
                'tSplVatType' => $aCompanyInfo['tCmpRetInOrEx'], // ภาษีรวมใน
                'tDocNo' => $tDocNo,
                'tDocKey' => 'TCNTPdtTboHD',
            ...
            $aDataMaster = array(
                ...
                'FTXthVATInOrEx' => $aCompanyInfo['tCmpRetInOrEx'], // ภาษีมูลค่าเพิ่ม 1:รวมใน, 2:แยกนอก
