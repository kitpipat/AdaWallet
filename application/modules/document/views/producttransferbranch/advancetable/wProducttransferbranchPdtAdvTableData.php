...
function FSvTBXPDTAddPdtIntoTableDT_PDTSerialorFashion(ptPdtDataFhn){
 ...
 var tOptionAddPdt = $('#ocmTBOptionAddPdt').val();
  $.ajax({
      type: "POST",
...
function FSvTBXPDTEditPdtIntoTableDT_PDTSerialorFashion(ptPdtDataFhn){
    console.log(ptPdtDataFhn);
    var aData = JSON.parse(ptPdtDataFhn);
  var ptXthDocNoSend = "";
  if ( $("#ohdTBRoute").val() == "TBXEventEdit" ) {
  ...
  $.ajax({
    ...
    function FSvTBXAddPdtIntoDocDTTempScan(ptPdtData){
        ...
        for(var i=0; i<nLen; i++){
            ...
            var nVat            = (parseFloat($('#ohdTbxVatRate').val())).toFixed(2);                               //ภาษีจากผู้จำหน่าย
