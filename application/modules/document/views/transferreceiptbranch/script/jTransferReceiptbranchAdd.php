<script type="text/javascript">
    ...
    function JSxTBIEventGetPdtIntDTBch(ptTBODocNo){
        var nStaSession = JCNxFuncChkSessionExpired();
            if(typeof(nStaSession) !== 'undefined' && nStaSession == 1){
              let tTBIBchCodeTo =  $('#oetTBIBchCodeTo').val();
              let tTBIBchCode =  $('#oetTBIBchCode').val();
        ...
        function JSvTWOGetShipAddrData(paInForCon){
         ...
         $('#obtTBIFrmBrowseShipAdd').unbind().click(function(){
            ...
            GrideView:{
                ...
                DisabledColumns     : [5],
                Perpage			    : 10,
                OrderBy			    : ['TCNTPdtIntDTBch.FDCreateOn DESC'],
            },
            CallBack: {
                ReturnType	: 'S',
    ...
    $('#oetTBIDocReferBrows').unbind().click(function(){
            ...
            oBrowseTBIBPdtIntBranchBrows.Where.Condition = ["AND TCNTPdtIntDTBch.FTXthBchTo = '" + tTBIBchCode + "' AND  ( ISNULL(TCNTPdtIntDTBch.FTXtdRvtRef,'') = '' OR  (ISNULL(TCNTPdtIntDTBch.FCXtdQty,0) - ISNULL(TCNTPdtIntDTBch.FCXtdQtyRcv,0))>0 )  "];
    ...
    function JCNvTBIBrowsePdt(){
        ...
        $.ajax({
            ...
            data: {
                ...
                SelectTier      : ["Barcode"],
                ShowCountRecord : 10,
                NextFunc        : "FSvTBXNextFuncB4SelPDT",
                ReturnType      : "M",
                SPL             : ['',''],
    ...
    function JSxTransferreceiptEventAddEdit(ptRoute){
        ...
        $("#ofmTransferreceiptFormAdd").validate({
            ...
            submitHandler: function (form) {
                var tItem = $('#odvTBodyPIPdtAdvTableList .xWPdtItem').length;
                if(tItem > 0){
                    $.ajax({
                        type    : "POST",
                        url     : ptRoute,
    ...
    var oBrowse_BCH = {
            ...
            NextFunc    :   {
                FuncName    :   'JSxSetDefauleWahouseBchCreate',
        ...
        function JSxSetDefauleWahouse(ptData){
            if(ptData == '' || ptData == 'NULL'){
                ...
                var oUpdChgBch = {
                    rtDocBchCode : rtDocBchCode,
                    rtDocNo      : $('#oetTBIDocNo').val(),
                    rtDocKey     : 'TCNTPdtTbiHD'
                }
    ...
    function JSxTBITransferReceiptPrintDoc(){
        var aInfor = [
			...
			{"BranchCode"   : '<?=FCNtGetAddressBranch($tTBIBchCode); ?>' }, // สาขาที่ออกเอกสาร
			{"DocCode"      : $('#oetTBIDocNo').val()  },
            {"DocBchCode"   : '<?=$tTBIBchCode?>' } // เลขที่เอกสาร
		];
    ...
    $('#ocmTBIptionAddPdt').on('change',function(){
		let nCountDataInTable = $('#otbTbxDocPdtAdvTableList tbody .xWPdtItem').length;
