  ...
  <div class="table-responsive">
      ...
      <table class="table table-striped xWPdtTableFont" id="otbDOCPdtTable">
        <thead>
          <tr class="xCNCenter">
            <th><?= language('document/transferwarehouseout/transferwarehouseout','tTWOTBChoose')?></th>
            ...
            <?php if((@$tXthStaApv == '') && @$tXthStaDoc != 3){?>
              <th><?= language('document/transferwarehouseout/transferwarehouseout','tTWOTBDelete')?></th>
              <th><?= language('document/transferwarehouseout/transferwarehouseout','tTWOTBEdit')?></th>
            <?php } ?>
          </tr>
        </thead>
        <tbody id="odvTBodyTWOPdt">
        <?php $nNumSeq = 0; ?>
  ...
  <div class="row">
    ...
    <div class="col-md-6">
        <div class="xWPageTwoPdt btn-toolbar pull-right">
            ...
            <button onclick="JSvTwoPdtClickPage('previous')" class="btn btn-white btn-sm" <?php echo $tDisabledLeft ?>> 
            ...
            <?php for($i=max($nPage-2, 1); $i<=max(0, min($aDataDT['rnAllPage'],$nPage+2)); $i++){?> 
                ...
                <button onclick="JSvTwoPdtClickPage('<?php echo $i?>')" type="button" class="btn xCNBTNNumPagenation <?php echo $tActive ?>" <?php echo $tDisPageNumber ?>><?php echo $i?></button>
            <?php } ?>
            <?php if($nPage >= $aDataDT['rnAllPage']){  $tDisabledRight = 'disabled'; }else{  $tDisabledRight = '-';  } ?>
            <button onclick="JSvTwoPdtClickPage('next')" class="btn btn-white btn-sm" <?php echo $tDisabledRight ?>> 
...
<!-- Modal Delete Items -->
<div class="modal fade" id="odvModalDelPdtTWO">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button id="osmConfirm" type="button" class="btn xCNBTNPrimery" onClick="JSoTWOPdtDelChoose('<?=@$nCurrentPage?>')"><?=language('common/main/main', 'tModalConfirm')?></button>
...
<?php  include("script/jTransferwarehouseoutPdtAdvTableData.php");?>
