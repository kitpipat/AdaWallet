...
var oCardShiftChangeBrowseNewCard{index} = function(ptNotCardCode) {
   ...
   //    tCondition = " AND TFNMCard.FTAgnCode ='<?php echo $this->session->userdata("tSesUsrAgnCode"); ?>' AND TFNMCard.FTAgnCode !='' ";
    ...
    let oOptions = {
        ...
        Where :{
            Condition : [" AND (TFNMCard.FTCrdStaShift = '2') AND (TFNMCardType.FTCtyStaPay = '"+tCtyStaPay+"') AND (TFNMCard.FTCrdStaActive = '1') AND (CONVERT(date, TFNMCard.FDCrdExpireDate) > CONVERT(date, GETDATE())) " + tNotIn + tWhereFCXsdAmt  + tCondition]
        },
        GrideView:{
