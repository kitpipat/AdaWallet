...
<div id="odvTabConditionRedeemHDFinal" >
    ...
    <div class="col-md-12 xGroupPingFinal" >
            <div class="col-md-6" >
                <label class="xCNLabelFrm"><?php echo language('document/conditionredeem/conditionredeem','tRDHClvInclude')?></label>
            </div>
            <div class="col-md-6">
                <label class="xCNLabelFrm"><?php echo language('document/conditionredeem/conditionredeem','tRDHClvExclude')?></label>
            </div>

            <div class="col-md-6" id="odvRddClvShow1" >
              
            </div>

            <div class="col-md-6" id="odvRddClvShow2" >
...
function JSxRddPdtGroupPageFinal(){
   ...
   //--กลุ่มสินค้า
    ...
    //--ระดับลูกค้าที่มีผล/ยกเว้น
    JSxRddClvGroupConditionRedeem();
}
...
function JSxRddClvGroupConditionRedeem(){

let nLenIn = $('input[name^="ohdRddClvCode["]').length
let aEchDataIn = JSxCreateArray(nLenIn,2);
//tRddClvname
$('input[name^="ohdRddClvCode["]').each(function(index){
let tRddClvname = $(this).attr('tRddClvname');
aEchDataIn[index][0]=tRddClvname;
});
//tRdhClvStaType
$('input[name^="ohdRdhClvStaType["]').each(function(index){
    let tRdhClvStaType = $(this).val();
    aEchDataIn[index][1]=tRdhClvStaType;
});
console.log(aEchDataIn);
...
for(i=0;i<aEchDataIn.length;i++){
        ...
        let tClvName   = aEchDataIn[i][0];
        let nType      = aEchDataIn[i][1];

        let tMarkUp ='<div class="col-md-12" >'+tClvName+'</div>';

        $("#odvRddClvShow"+nType).append(tMarkUp);
}

if($('#odvRddClvShow1').html()==''){
    $('#odvRddClvShow1').html('<?php echo language('document/conditionredeem/conditionredeem','tRDHAllInclude')?>');
}

if($('#odvRddClvShow2').html()==''){
    $('#odvRddClvShow2').html('<?php echo language('document/conditionredeem/conditionredeem','tRDHALlExclude')?>');
}
