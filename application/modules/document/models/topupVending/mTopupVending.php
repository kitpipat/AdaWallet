...
class mTopupVending extends CI_Model
{
    ...
    public function FSaMGetPdtLayoutInTmp($paParams = [])
    {
        ...
        $tSQL = "
            SELECT c.* FROM(
                SELECT  ROW_NUMBER() OVER(ORDER BY FNCabSeqForTWXVD ASC, FNLayRowForTWXVD ASC, FNLayColForTWXVD ASC) AS FNRowID,* FROM
                    ...
                    /* LEFT JOIN TCNTPdtStkBal PSB ON TMP.FTPdtCode = PSB.FTPdtCode AND FTWahCode='00006' */
        ...
        //FCXtdQty = จำนวนเติม
        if($paParams['tTypePage'] == 'TopupVendingEventAdd'){
            //เฉพาะสินค้าที่ต้องเติม
            if($paParams['tStaShwPdt'] == '0'){
                $tSQL .= " AND TMP.FCStkQty != TMP.FCLayColQtyMaxForTWXVD";
            }

            //แสดงเฉพาะสินค้าที่มีในสต็อก
            if($paParams['tStaShwPdtInStk'] == 'true'){
                // $tSQL .= " AND TMP.FCXtdQty != 0 ";
                $tSQL .= " AND TMP.FCXtdQtyAll > 0 ";
            }
        }

        // Update : Napat(Jame) 01/09/2020 เปิดการมองเห็นสินค้าทั้งหมด และไป Control การมองเห็นที่หน้าจอ
        // if($paParams['tTypePage'] == 'TopupVendingEventEdit'){
        //     $tSQL .= "AND TMP.FCXtdQty != 0";
        // }else{
        //     $tSQL .= "AND TMP.FCStkQty != TMP.FCLayColQtyMaxForTWXVD AND TMP.FCXtdQty != 0";
        // }

        //ถ้าสินค้าที่เหลืออยู่ เท่ากับ จำนวนที่เติมได้สูงสุด จะต้องไม่เอาไปแสดงให้เติม
        //supawat 19 / 08 / 2020
        //AND TMP.FCStkQty != TMP.FCLayColQtyMaxForTWXVD
    ...
    public function FSaMGetWahInDT($paParams = [])
    {
        ...
        $tSQL = "
            ...
            LEFT JOIN TCNMWaHouse_L WAHL WITH(NOLOCK) ON WAHL.FTWahCode = DT.FTXthWhFrm AND DT.FTBchCode = WAHL.FTBchCode AND WAHL.FNLngID = '$nLngID'
    ...
    public function FSaMPdtlayoutToTemp($paParams = [])
    {
        ...
        $tWahCode           = $this->FStMGetWahCodeByRefCode(['tRefCode' => $tPosCode , 'tBCHCode' => $tBchCode]); // คลังสินค้าของ ตู้ขายที่เลือก
        ...
        $tSQL .= "  
            SELECT 
                ...
                FTWahStaChkStk      AS FTWahStaChkStk
            ...
            LEFT JOIN TCNMWaHouse       WAH     WITH(NOLOCK)  ON WAH.FTWahCode = PDTLAY.FTWahCode AND WAH.FTBchCode = PDTLAY.FTBchCode
            WHERE PDTLAY.FTBchCode = '$tBchCode' AND PDTLAY.FTShpCode = '$tShpCode' AND PDTLAY.FTWahCode IN ($tWahCodeInShop)
    ...
    public function FSaMDTToTemp($paParams = [])
    {
        ...
        $tSQL .= "  
            SELECT
                ...
                FTWahStaChkStk      AS FTWahStaChkStk
            FROM TVDTPdtTwxDT DT WITH(NOLOCK)
            LEFT JOIN TVDTPdtTwxHD      HD      WITH(NOLOCK)    ON HD.FTXthDocNo = DT.FTXthDocNo AND HD.FTBchCode = DT.FTBchCode   
            LEFT JOIN TVDMPdtLayout     PDTLAY  WITH(NOLOCK)    ON PDTLAY.FTBchCode = DT.FTBchCode      AND PDTLAY.FTShpCode = HD.FTXthShopTo AND PDTLAY.FNCabSeq = DT.FNCabSeq AND PDTLAY.FNLayRow = DT.FNLayRow AND PDTLAY.FNLayCol = DT.FNLayCol
            LEFT JOIN TVDMShopCabinet_L SHPCABL WITH(NOLOCK)    ON SHPCABL.FNCabSeq = PDTLAY.FNCabSeq   AND SHPCABL.FTBchCode = PDTLAY.FTBchCode AND SHPCABL.FTShpCode = PDTLAY.FTShpCode AND SHPCABL.FNLngID = $nLngID
    ...
    {
        $this->db->set('FCXtdQtyAll', $paParams['cQtyAll']);
    ...
    // create by witsarut 030920
    public function FSbDeleteMultiInTmpBySeq($aDataWhere){
        try{
            $this->db->trans_begin();
            $this->db->where_in('FNXtdSeqNo', $aDataWhere['FNXtdSeqNo']);
            $this->db->where_in('FTPdtCode', $aDataWhere['FTPdtCode']);
            $this->db->where_in('FTSessionID', $aDataWhere['tUserSessionID']);
            $this->db->delete('TCNTDocDTTmp');
    ...
    public function FSxMTVDDelPdtTopupZero($paData){
        $this->db->where('FCXtdQty', 0);
        $this->db->where('FTSessionID', $paData['tUserSessionID']);
