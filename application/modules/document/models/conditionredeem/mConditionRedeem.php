...
class mConditionRedeem extends CI_Model {
    ...
    //Functionality : Function Insert TFNTCouponHDCstLev
    //Parameters : function parameters
    //Creator : 27/02/2020 Nale
    //Last Modified : -
    //Return : Status Add
    //Return Type : array
    public function FSaMRDHAddUpdateConditionRedeemHDCstLev($paDataDT,$paDataHD){

        $tSQLDelete = "DELETE FROM TARTRedeemHDCstLev WHERE 1=1 AND TARTRedeemHDCstLev.FTBchCode = '".$paDataHD['FTBchCode']."' AND  TARTRedeemHDCstLev.FTRdhDocNo = '".$paDataHD['FTRdhDocNo']."' ";
        $oQuery = $this->db->query($tSQLDelete);

        if(isset($paDataDT['aRddClvCode']) && !empty($paDataDT['aRddClvCode'])){
            // Loop Add/Update 
            foreach($paDataDT['aRddClvCode'] AS $nKey => $aValue){
                $tSQLInsert = "";
                $tSQLInsert = " INSERT INTO TARTRedeemHDCstLev ( FTBchCode,FTRdhDocNo,FTClvCode,FTRdhStaType ) VALUES (
                                    '".$paDataHD['FTBchCode']."',
                                    '".$paDataHD['FTRdhDocNo']."',
                                    '".$aValue."',
                                    '".$paDataDT['aRdhClvStaType'][$nKey]."'
                                )";
                $oQuery = $this->db->query($tSQLInsert);
            }
        }
    ...
    //Return : data
    //Return Type : Array
    public function FSaMRDHGetDataCstLevByID($paData){
        $nLngID     = $paData['FNLngID'];
        $tRdhDocNo  = $paData['FTRdhDocNo'];

        $aConditionRedeemCstPri = $this->db->where('FTRdhDocNo',$tRdhDocNo)
        ->join('TCNMCstLev_L' ,'TARTRedeemHDCstLev.FTClvCode=TCNMCstLev_L.FTClvCode AND TCNMCstLev_L.FNLngID='.$nLngID,'left')
        ->get('TARTRedeemHDCstLev')
        ->result_array();
