...
class mPromotion extends CI_Model
{
     ...
     * Return Type : Array
     */
    public function FSaMTempToPdtPmtHDRcv($paParams = [])
    {
        ...
        // ทำการลบ ใน TCNTPdtPmtHDChn ก่อนการย้าย Temp ไป TCNTPdtPmtHDChn
        $this->db->where('FTPmhDocNo', $tDocNo);
        $this->db->delete('TCNTPdtPmtHDPay');

        $tSQL = "   
            INSERT TCNTPdtPmtHDPay
                (FTBchCode,
                FTPmhDocNo,
                FTRcvCode,
                FTPmhStaType)
    ...
    public function FSaMPdtPmtHDRcvToTemp($paParams = [])
    {
        ...
        $tSQL .= "  
            ...
            HDCSTRCV.FTBchCode,
                'PMTDOCTEMP' AS FTPmhDocNo,
                HDCSTRCV.FTRcvCode,
                HDCSTRCV.FTPmhStaType,
                '$tUserSessionDate' AS FDCreateOn,
                '$tUserSessionID' AS FTSessionID
            FROM TCNTPdtPmtHDPay HDCSTRCV WITH(NOLOCK)
            LEFT JOIN TFNMRcv_L RCVL ON RCVL.FTRcvCode = HDCSTRCV.FTRcvCode  AND RCVL.FNLngID = $nLngID
            WHERE HDCSTRCV.FTPmhDocNo = '$tDocNo'
            ORDER BY RCVL.FTRcvName ASC
        ";
     ...
     * Return Type : Array
     */
    public function FSaMTempToPdtPmtHDCst($paParams = [])
    {
    ...
    public function FSaMPdtPmtHDCstToTemp($paParams = [])
    {
        ...
        $tSQL .= "  
            ...
            HDCSTCST.FTBchCode,
                ...
                '$tUserSessionDate' AS FDCreateOn,
                '$tUserSessionID' AS FTSessionID
            FROM TCNTPdtPmtHDCstLev HDCSTCST WITH(NOLOCK)
            LEFT JOIN TCNMCstLev_L CLVL ON CLVL.FTClvCode = HDCSTCST.FTClvCode  AND CLVL.FNLngID = $nLngID
            WHERE HDCSTCST.FTPmhDocNo = '$tDocNo'
            ORDER BY CLVL.FTClvName ASC
        ";
