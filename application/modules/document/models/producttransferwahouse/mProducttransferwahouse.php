...
class mProducttransferwahouse extends CI_Model
{
    ...
    */
    ...
    public function FSaMTFWGetDT($paData)
    {
        ...
        $tSQL   = "SELECT c.* FROM(
                        SELECT  ROW_NUMBER() OVER(ORDER BY FTXthDocNo ASC) AS FNRowID,* FROM
                            ...
                            LEFT JOIN TCNMPdt PDT WITH(NOLOCK) ON TWFDT.FTPdtCode = PDT.FTPdtCode
                            ";
    ...
    public function FSxMClearPdtInTmp()
    {
        ...
        $tSQL = "DELETE FROM TCNTDocDTFhnTmp WHERE FTSessionID = '" . $this->session->userdata('tSesSessionID') . "' AND FTXthDocKey = 'TCNTPdtTwxHD'";
        $this->db->query($tSQL);
    }
