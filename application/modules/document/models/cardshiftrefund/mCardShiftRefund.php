...
class mCardShiftRefund extends CI_Model
{
    ...
    {
        $tCardShiftRefundDocNo = $paData['FTXshDocNo'];
        $nLngID = $paData['FNLngID'];
        $tSQL = "
            SELECT
                CRDHD.FTXshDocNo AS rtCardShiftRefundDocNo,
                CRDHD.FTBchCode AS rtCardShiftRefundBchCode,
                CRDHD.FNXshDocType AS rtCardShiftRefundDocType,
                CRDHD.FDXshDocDate AS rtCardShiftRefundDocDate,
                /*CRDHD.FTUsrCode AS rtCardShiftRefundUsrCode,*/
                /*CRDHD.FTXshApvCode AS rtCardShiftRefundApvCode,*/
                CRDHD.FNXshCardQty AS rtCardShiftRefundCardQty,
                CRDHD.FCXshTotal AS rtCardShiftRefundAmtTP,
                CRDHD.FCXshTotal AS rtCardShiftRefundTotalTP,
                CRDHD.FTXshStaPrcDoc AS rtCardShiftRefundStaPrcDoc,
                CRDHD.FTXshStaDoc AS rtCardShiftRefundStaDoc,
                CRDHD.FTXshStaDelMQ AS rtCardShiftRefundStaDelMQ,
                CRDHD.FTUsrCode	AS FTUsrCode,
                CRDHD.FTCreateBy AS FTUsrCodeCreate,
                USRC.FTUsrName AS FTUsrNameCreate,
                CRDHD.FTXshApvCode AS FTUsrCodeApv,
                USRA.FTUsrName AS FTUsrNameApv,
                BCHL.FTBchCode,
                BCHL.FTBchName
            FROM [TFNTCrdTopUpHD] CRDHD
            LEFT JOIN TCNMUser_L USRC WITH(NOLOCK) ON CRDHD.FTCreateBy = USRC.FTUsrCode AND USRC.FNLngID = $nLngID
            LEFT JOIN TCNMUser_L USRA WITH(NOLOCK) ON CRDHD.FTXshApvCode = USRA.FTUsrCode AND USRA.FNLngID = $nLngID
            LEFT JOIN TCNMBranch_L BCHL ON CRDHD.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID = '$nLngID'
            WHERE 1=1
        ";
        $tSQL .= " AND CRDHD.FTXshDocNo = '$tCardShiftRefundDocNo'";
        $tSQL .= " AND CRDHD.FNXshDocType = '11'"; // 4: Refund
    ...
    public function FSaMCardShiftRefundList($ptAPIReq, $ptMethodReq, $paData)
    {
            ...
            SELECT c.* FROM(
                ...
                FROM
                    (SELECT DISTINCT
                        ...
                        CRDHD.FTXshDocNo AS rtCardShiftRefundDocNo,
                        CRDHD.FDXshDocDate AS rtCardShiftRefundDocDate,
                        CRDHD.FNXshCardQty AS rtCardShiftRefundCshCardQty,
                        CRDHD.FTXshStaDoc AS rtCardShiftRefundCshStaDoc,
                        CRDHD.FTXshStaApv AS rtCardShiftRefundCshStaApv,
                        CRDHD.FTXshStaPrcDoc AS rtCardShiftRefundCshStaPrcDoc,
                        CRDHD.FDCreateOn AS rtCardShiftRefundFDCreateOn
                    FROM [TFNTCrdTopUpHD] CRDHD WITH(NOLOCK)
                    WHERE 1=1
        ";

        $tSQL .= " AND CRDHD.FNXshDocType = '11'"; // 5: Refund    

        if ($this->session->userdata("tSesUsrLevel") != "HQ") {
            $tUsrBchCode = $this->session->userdata("tSesUsrBchCodeMulti");
            $tSQL .= " AND CRDHD.FTBchCode IN ($tUsrBchCode)";
        }
        ...
        $tSearchStaDocAct = $oAdvanceSearch->tSearchStaAct;
    ...
    public function FSoMCardShiftRefundCheckDuplicate($ptCardShiftRefundCode)
    {
        $tSQL = "
            ...
            FROM TFNTCrdTopUpHD
            WHERE FTXshDocNo = '$ptCardShiftRefundCode'
        ";
    ...
    public function FSaMCardShiftRefundAddUpdateHD($paData)
    {
        ...
        $this->db->set('FCXshTotal', $paData['FCXshTotal']);
    ...
    public function FSnMCardShiftRefundDel($paData)
    {
        ...
        $this->db->where('FTXshDocNo', $paData['tDocNo']);
        $this->db->where('FNXshDocType', $paData['tDocType']);
        $this->db->where('FTBchCode', $paData['tBchCode']);
    ...
    public function FSaMCardShiftRefundGetDTByDocNo($paData)
    {
        ...
        $tSQL = "
            SELECT
                CRDDT.FTXshDocNo AS rtCardShiftRefundDocNo,
                CRDDT.FTBchCode AS rtCardShiftRefundBchCode,
                CRDDT.FTCrdCode AS rtCardShiftRefundCrdCode,
                CRDDT.FNXsdSeqNo AS rtCardShiftRefundSeqNo,
                CRDDT.FCXsdAmt AS rtCardShiftRefundCrdTP,
                CRDDT.FCXsdAmt AS rtCardShiftRefundCrdValue,
                (CASE 
                    WHEN ((CARDB.CashIn + CARDB.Promotion) - (CARDB.Payment + CARDB.NotReturn)) < 0 THEN 0
                    ELSE ((CARDB.CashIn + CARDB.Promotion) - (CARDB.Payment + CARDB.NotReturn))
                END) AS rtCardShiftTopUpCrdBal,
                ((CARDB.CashIn + CARDB.Promotion) - CARDB.DepositCrd - CARDB.DepositPdt - CARDB.Payment) AS rtCardShiftTopUpValidCrdVal,
                CRDDT.FCXsdAmtPmt AS rtCardShiftTopUpAmtPmt,
                CRDDT.FTXsdRmk AS rtCardShiftRefundRmk,
                CRD.FTCrdStaShift AS rtCardShiftRefundCrdStaShift,
                CRD.FTCrdStaActive AS rtCardShiftRefundCrdStaActive,
                CRDT.FTCtyStaShift AS rtCardShiftRefundCrdStaType,
                CRD.FDCrdExpireDate AS rtCardShiftRefundCrdExpireDate,
                CRDL.FTCrdName AS rtCardShiftRefundCrdName,
                CARDB.Promotion AS Pro
    ...
    public function FSaMCardShiftRefundGetCardById($paData)
    {
        ...
        $tSQL = "
            SELECT
                ...
                CRDT.FTCtyStaShift AS rtCardShiftRefundStaType,
    ...
    public function FSaMCardShiftRefundListScanner($paData){
            ...
            $tSQL       = "SELECT
                        ...
                        FROM
                            (
                                ...
                                AND TFNMCard.FTCrdCode IN (
                                    SELECT CB.FTCrdCode
                                    FROM (
                                        SELECT CRD.FTCrdCode,
                                            SUM ( CASE WHEN CRDB.FTCrdTxnCode = '001' THEN ISNULL(CRDB.FCCrdValue, 0) ELSE 0 END) CashIn,
                                            SUM ( CASE WHEN CRDB.FTCrdTxnCode = '002' THEN ISNULL(CRDB.FCCrdValue, 0) ELSE 0 END) Promotion,
                                            SUM ( CASE WHEN CRDB.FTCrdTxnCode = '003' THEN ISNULL(CRDB.FCCrdValue, 0) ELSE 0 END) DepositCrd,
                                            SUM ( CASE WHEN CRDB.FTCrdTxnCode = '004' THEN ISNULL(CRDB.FCCrdValue, 0) ELSE 0 END) DepositPdt,
                                            SUM ( CASE WHEN CRDB.FTCrdTxnCode = '005' THEN ISNULL(CRDB.FCCrdValue, 0) ELSE 0 END) NotReturn,
                                            SUM ( CASE WHEN CRDB.FTCrdTxnCode = '006' THEN ISNULL(CRDB.FCCrdValue, 0) ELSE 0 END) Payment
                                        FROM TFNMCard CRD WITH (NOLOCK)
                                        LEFT JOIN TFNMCardBal CRDB WITH (NOLOCK) ON CRDB.FTCrdCode = CRD.FtCrdCode
                                        GROUP BY CRD.FTCrdCode
                                    ) CB
