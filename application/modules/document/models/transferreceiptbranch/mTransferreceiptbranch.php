...
class mTransferreceiptbranch extends CI_Model {
    ...
    public function FSaMTBIList($paData){
        ...
        if(@$tSearchList != ''){
            $tSQL .= " AND ((TBI.FTXthDocNo LIKE '%$tSearchList%') OR (TBI.FTXthRefInt LIKE '%$tSearchList%') OR (BCHL.FTBchName LIKE '%$tSearchList%') OR (CONVERT(CHAR(10),TBI.FDXthDocDate,103) LIKE '%$tSearchList%'))";
        }
        ...
        if(!empty($tSearchStaDoc) && ($tSearchStaDoc != "0")){
            if ($tSearchStaDoc == 3) {
                ...
                $tSQL .= " AND ISNULL(TBI.FTXthStaApv,'') = '' AND TBI.FTXthStaDoc != '3'";
            } elseif ($tSearchStaDoc == 1) {
                $tSQL .= " AND TBI.FTXthStaApv = '$tSearchStaDoc'";
            }
        }
    ...
    // Check User Login Branch
      if($this->session->userdata('tSesUsrLevel')!='HQ'){
        ...
        @$tSearchList   = $aAdvanceSearch['tSearchAll'];
        if(@$tSearchList != ''){
            $tSQL .= " AND ((TBI.FTXthDocNo LIKE '%$tSearchList%') OR (TBI.FTXthRefInt LIKE '%$tSearchList%') OR (BCHL.FTBchName LIKE '%$tSearchList%') OR (TBI.FDXthDocDate LIKE '%$tSearchList%'))";
        }
    ...
    public function FSaMTBIInsertPDTToTemp($paDataPdtMaster,$paDataPdtParams){
        ...
        if($paDataPdtParams['nTBIptionAddPdt']==1){
    ...
    public function FSxMTBIMoveDTToDTTemp($paDataWhere){
        ...
        $tSQL   = " INSERT INTO TCNTDocDTTmp (
                        ...
                        FCXtdQtySet, FTXtdPdtStaSet, FTXtdRmk, FTXtdBchRef,FTTmpStatus,
...
public function FSoMTBIEventGetPdtIntDTBch($pData){ 
    ...
    $tSqlDocTmp = " SELECT 
                        ...
                        (ISNULL(ITB.FCXtdQty,0) - ISNULL(ITB.FCXtdQtyRcv,0))*ITB.FCXtdFactor AS FCXtdQtyAll
                    FROM TCNTPdtIntDTBch ITB WITH(NOLOCK)
                    WHERE ITB.FTXthDocNo = '$tTBODocNo'
                    AND ITB.FTXthBchTo = '$tTBIBchCodeTo'
                    AND  ( ISNULL(ITB.FTXtdRvtRef,'') = '' OR  (ISNULL(ITB.FCXtdQty,0) - ISNULL(ITB.FCXtdQtyRcv,0))>0 ) 
    ...
    public function FSoMTBIEventGetPdtIntDTFhnBchToTemp($pData){ 

        $tTBODocNo =  $pData['tTBODocNo'];
        $tTBIDocNo =  $pData['tTBIDocNo'];
        $tTBIBchCode =  $pData['tTBIBchCode'];
        $tTBIBchCodeTo =  $pData['tTBIBchCodeTo'];
        $tTBISesUsername =  $pData['tTBISesUsername'];
        $tTBISessionID =  $pData['tTBISessionID'];
    
        $tSqlDocTmp = " INSERT INTO TCNTDocDTFhnTmp 
                            (
                            FTBchCode,
                            FTXshDocNo,
                            FNXsdSeqNo,
                            ...
                            SELECT
                                '$tTBIBchCode' AS FTBchCode,
                                '$tTBIDocNo' AS FTXshDocNo,
                                ITB.FNXtdSeqNo,
                                'TCNTPdtTbiHD' AS FTXthDocKey,
                                ITFN.FTPdtCode,
                                ITFN.FTFhnRefCode,
                                (ISNULL(ITFN.FCXtdQty,0) - ISNULL(ITFN.FCXtdQtyRcv,0)) AS FCXtdQty,
                                '$tTBISessionID' AS FTSessionID
                            FROM
                                TCNTPdtIntDTBch ITB WITH (NOLOCK)
                            INNER JOIN TCNTPdtIntDTFhnBch ITFN WITH (NOLOCK) ON ITB.FTBchCode = ITFN.FTBchCode AND ITB.FTXthDocNo = ITFN.FTXthDocNo AND ITB.FNXtdSeqNo = ITFN.FNXtdSeqNo
                            WHERE
                                ITB.FTXthDocNo = '$tTBODocNo'
                            AND ITB.FTXthBchTo = '$tTBIBchCodeTo'
                            AND ( ISNULL(ITB.FTXtdRvtRef,'') = '' OR  (ISNULL(ITB.FCXtdQty,0) - ISNULL(ITB.FCXtdQtyRcv,0))>0 ) 
                            ORDER BY
                                ITB.FNXtdSeqNo ASC
    ...
    //Return : Status Add/Update HD Ref
    //Return Type : array
    public function FSaMTBICheckRefDocRecive($paData){

        $tXthBchCode = $paData['tXthBchCode'];
        ...
        $tSQL ="SELECT 
                    ...
                    INTBCH.FCXtdQtyRcv,
                    INTBCH.FCXtdQtyAll,
                    ISNULL(FCXtdQty,0) - ISNULL(FCXtdQtyRcv,0) AS QtyDiff
                FROM
                    TCNTPdtIntDTBch INTBCH WITH (NOLOCK)
                WHERE 1=1
                AND INTBCH.FTXthBchTo = (SELECT TBI.FTXthBchTo FROM TCNTPdtTbiHD TBI WHERE FTBchCode = '$tXthBchCode' AND  FTXthDocNo = '$FTXthDocNo')
                AND INTBCH.FTXthDocNo = (SELECT TBI.FTXthRefInt FROM TCNTPdtTbiHD TBI WHERE FTBchCode = '$tXthBchCode' AND  FTXthDocNo = '$FTXthDocNo')
                AND ( ISNULL(INTBCH.FTXtdRvtRef,'') = '' OR  (ISNULL(INTBCH.FCXtdQty,0) - ISNULL(INTBCH.FCXtdQtyRcv,0))>0 )  ";
