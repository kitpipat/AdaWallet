...
class cCustomer extends MX_Controller {
    ...
    public function FSvCSTAddPage(){
        ...
        $tAlwAdjLevelMem = $this->db->where('FTSysCode','AMQMember')->where('FTSysSeq',1)->get('TSysConfig')->row_array()['FTSysStaUsrValue'];//ตั้งค่าใช้งาน Member
        $aClvDefault = $this->mCustomer->FSaMCSTGetConfigCstLevDefault();//Default Code ระดับลูกค้า

        if($aClvDefault['rtCode']=='1'){
            $tClvCodeDefault = $aClvDefault['raData']['FTSysStaUsrValue'];
            $tClvNameDefault = $aClvDefault['raData']['FTClvName'];
        }else{
            $tClvCodeDefault =  '';
            $tClvNameDefault = '';
        }
        ...
        $aDataAdd = array(
            ...
            'tClvCodeDefault' => $tClvCodeDefault,
            'tClvNameDefault' => $tClvNameDefault,
        );
