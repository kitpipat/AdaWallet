  ...
  /**
    ...
    public function FScMCSTGetInfoMemberConFigSpc($ptAgnCode,$nSeq){
        ...
        $tSQL ="SELECT CONF.*,CGP.FTCgpName FROM (
            ...
            WHERE
                CON.FTSysCode = 'AMQMember'
            AND CON.FTSysApp = 'MB'
            AND CON.FTSysKey = 'Member'
            AND CON.FTSysSeq IN ('$nSeq')
            ) AS CONF
     ...
     * Return Type : array
     */
    public function FSaMCSTGetConfigCstLevDefault(){
        $nLangEdit      = $this->session->userdata("tLangEdit");
        $tSQL ="SELECT
                    CFG.FTSysStaUsrValue,
                    CLV_L.FTClvName
                FROM
                    TSysConfig CFG WITH (NOLOCK)
                LEFT OUTER JOIN TCNMCstLev_L CLV_L WITH (NOLOCK) ON CFG.FTSysStaUsrValue = CLV_L.FTClvCode
                AND CLV_L.FNLngID = $nLangEdit
                WHERE
                    CFG.FTSysCode = 'tCN_CstLevDef'
                AND CFG.FTSysSeq = '1'";
