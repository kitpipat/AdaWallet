            ...
            <table id="otbClrDataList" class="table table-striped"> <!-- เปลี่ยน -->
                <thead>
                    <tr>
                        ...
                        <?php endif; ?>
                        <?php if($aAlwEventPdtColor['tAutStaFull'] == 1 || ($aAlwEventPdtColor['tAutStaEdit'] == 1 || $aAlwEvertClrCodeLef2ntPdtColor['tAutStaRead'] == 1))  : ?>
                    ...
                    <?php if($aClrDataList['rtCode'] == 1 ):?>
                        <?php foreach($aClrDataList['raItems'] AS $nKey => $aValue):?>
                            <tr class="text-center xCNTextDetail2 otrPdtClr" id="otrPdtClr<?=$nKey?>" data-code="<?=$aValue['rtClrCode']?>" data-name="<?=$aValue['rtClrName']?>">
                                <?php if($aAlwEventPdtColor['tAutStaFull'] == 1 || $aAlwEventPdtColor['tAutStaDelete'] == 1) : ?>
                                    <?php
                                    if($aValue['rtClrCodeLef'] != '' || $aValue['rtClrCodeLef2'] != ''){
                                ...
                                <td class="<?=$tDisableTD?>">
                                    <!-- เปลี่ยน -->
                                    <img class="xCNIconTable <?php echo $tDisableImg; ?>" src="<?php echo  base_url().'/application/modules/common/assets/images/icons/delete.png'?>" onClick="JSoPdtClrDel('<?=$nCurrentPage?>','<?=$aValue['rtClrName']?>','<?php echo $aValue['rtClrCode']?>','<?= language('common/main/main','tModalConfirmDeleteItemsYN')?>')">
                                </td>
                                <?php endif; ?>
