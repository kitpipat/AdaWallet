            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtPszCode DESC) AS rtRowID,* FROM
                                    (SELECT DISTINCT
                                        ...
                                        PSZ.FDCreateOn,
                                        PDT.FTPszCode   AS rtPszCodeLef,
                                        PDTCLS.FTPszCode   AS rtPszCodeLef2
                                    FROM [TCNMPdtSize] PSZ
                                    LEFT JOIN [TCNMPdtSize_L]  PSZ_L ON PSZ.FTPszCode = PSZ_L.FTPszCode AND PSZ_L.FNLngID = $nLngID
                                    LEFT JOIN (SELECT DISTINCT FTPszCode FROM TCNMPdtPackSize WITH(NOLOCK) ) PDT ON PDT.FTPszCode  = PSZ.FTPszCode
                                    LEFT JOIN (SELECT DISTINCT FTPszCode FROM TFHMPdtColorSize WITH(NOLOCK) ) PDTCLS ON PDTCLS.FTPszCode  = PSZ.FTPszCode
                                    WHERE 1=1 ";
