        ...
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtPplCode DESC) AS rtRowID,* FROM
                                    (SELECT DISTINCT
                                        CSL.FTPplCode  AS rtPplCodeLef,
                                        BCH.FTPplCode  AS rtPplCodeLef2,
                                        MER.FTPplCode  AS rtPplCodeLef3,
                                        AGN.FTPplCode  AS rtPplCodeLef4,
                                        PPL.FTPplCode   AS rtPplCode,
                                        PPL_L.FTPplName AS rtPplName,
                                        PPL.FDCreateOn,
                                        PPL.FTAgnCode       AS rtAgnCode,
                                        AGNL.FTAgnName      AS rtAgnName
                                    FROM [TCNMPdtPriList] PPL WITH(NOLOCk)
                                    LEFT JOIN [TCNMPdtPriList_L]  PPL_L WITH(NOLOCk) ON PPL.FTPplCode = PPL_L.FTPplCode AND PPL_L.FNLngID = $nLngID
                                    LEFT JOIN [TCNMAgency_L] AGNL WITH(NOLOCk) ON PPL.FTAgnCode = AGNL.FTAgnCode AND AGNL.FNLngID = $nLngID
                                    LEFT JOIN (SELECT DISTINCT FTPplCode FROM TCNMCstLev WITH(NOLOCk) ) CSL ON PPL.FTPplCode = CSL.FTPplCode
                                    LEFT JOIN (SELECT DISTINCT FTPplCode FROM TCNMBranch WITH(NOLOCk) ) BCH ON PPL.FTPplCode = BCH.FTPplCode
                                    LEFT JOIN (SELECT DISTINCT FTPplCode FROM TCNMMerchant WITH(NOLOCk) ) MER ON PPL.FTPplCode = MER.FTPplCode
                                    LEFT JOIN (SELECT DISTINCT FTPplCode FROM TCNMAgency WITH(NOLOCk) ) AGN ON PPL.FTPplCode = AGN.FTPplCode
                                    WHERE 1=1 ";
