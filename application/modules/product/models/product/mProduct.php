...
class mProduct extends CI_Model
{
    ...
    public function FSaMPDTGetDataTable($paData)
    {
        ...
        if ($tSesUsrLevel != 'HQ') {
            ...
            $tWHEREPermission_BCH .= "     PDLSPC.FTAgnCode = '$tSesUsrAgnCode'";

            if(!empty($tSessionMerCode)){ //กรณีผู้ใช้ผูก Mer จะเห็นสินค้าภายใต้ Mer
                $tWHEREPermission_BCH .= " AND PDLSPC.FTMerCode = '$tSessionMerCode'";
            }
            ...
            if(!empty($tSessionShopCode)){ //กรณีผู้ใช้ผูก Shp จะเห็นสินค้าภายใต้ Shp
                $tWHEREPermission_BCH .= " AND PDLSPC.FTShpCode IN ($tSessionShopCode) ";
            }
                $tWHEREPermission_BCH .= " ) ";
       /* |-------------------------------------------------------------------------------------------| */
        ...
        //---------------------- การมองเห็นสินค้าระดับสาขา (สำหรับผู้ใช้ระดับร้านค้า)--------------------------//
            if(!empty($tSessionShopCode)){  
                ...
                $tWHEREPermission_BCH .= "     PDLSPC.FTAgnCode = '$tSesUsrAgnCode'";
                $tWHEREPermission_BCH .= " AND PDLSPC.FTMerCode = ''";
                $tWHEREPermission_BCH .= " AND PDLSPC.FTBchCode IN ($tSessionBchCode) ";
                $tWHEREPermission_BCH .= " AND ISNULL(PDLSPC.FTShpCode,'') = ''"   ;
                $tWHEREPermission_BCH .= " ) ";
      ...
      /* |-------------------------------------------------------------------------------------------| */
       ...
       /* |-------------------------------------------------------------------------------------------| */
        ...
        $aSpcWhereTableMaster =  array(
            1 => " AND ( PDT.FTPdtCode  = '$tSearch' ) ", //รหัสสินค้า
            2 => " AND ( UPPER(PDTL.FTPdtName)  COLLATE THAI_BIN    LIKE UPPER('%$tSearch%') ) ",  //หาชื่อสินค้า
            3 => " AND ( PBAR.FTBarCode  = '$tSearch' ) ",  //หาบาร์โค้ด
        ...
        if($paData['nStaTopPdt']==1){
            $tSQLPdtMaster .="TOP ".$paData['nRow'];
        }
        // if ($paData['nPage'] == 1 && $tSesUsrLevel != 'HQ') {
        ...
        // die();
            ...
            $tQueryCountRow .= " WHERE ISNULL(PDT.FTPdtCode,'') != ''
                            " . $tPdtForSysLine1 . "
                            " . $tWHEREPermission_BCH . "
                            " . $tWHEREPermission_SHP . "
                            ";
            if (!empty($tSearch)) {
                $tQueryCountRow .= $aSpcWhereTableMaster[$nSearchProductType];
            }

            if($paData['nStaTopPdt']==2){
                    if ($paData['nPage'] == 1) {
                        $tQueryCount = "DECLARE @TotalProduct INT

                            SET @TotalProduct = (
                            $tQueryCountRow
                            ) 
                            ";
                        $tQueryCountSelect = " @TotalProduct AS rtCountData,";
                }else{
                        $tQueryCount ="DECLARE @TotalProduct INT";
                        $tQueryCountSelect = "";
                }
    ...
    //                 WHERE
    //                     PDT.FNRowID > $aRowLen[0]
    //                 AND PDT.FNRowID <= $aRowLen[1] ";
    //     if (!empty($tSearch)) {
