            ...
            $tSQL       = "SELECT c.* FROM(
                                SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtClrCode DESC) AS rtRowID,* FROM
                                    (SELECT DISTINCT
                                        ...
                                        CLR.FDCreateOn,
                                        PDT.FTClrCode   AS rtClrCodeLef,
                                        PDTCLS.FTClrCode   AS rtClrCodeLef2
                                    FROM [TCNMPdtColor] CLR
                                    LEFT JOIN [TCNMPdtColor_L]  CLR_L ON CLR.FTClrCode = CLR_L.FTClrCode AND CLR_L.FNLngID = $nLngID
                                    LEFT JOIN (SELECT DISTINCT FTClrCode FROM TCNMPdtPackSize WITH(NOLOCK) ) PDT ON PDT.FTClrCode  = CLR.FTClrCode
                                    LEFT JOIN (SELECT DISTINCT FTClrCode FROM TFHMPdtColorSize WITH(NOLOCK) ) PDTCLS ON PDTCLS.FTClrCode  = CLR.FTClrCode
                                    WHERE 1=1 ";
