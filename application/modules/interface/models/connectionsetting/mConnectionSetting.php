...
class mConnectionSetting extends CI_Model {
    ...
    public function FSaMCSSAddUpdateMaster($paData){
        try{
            ...
            $this->db->where("ISNULL(FTAgnCode,'')", $paData['FTAgnCode']);
            $this->db->where('FTBchCode', $paData['FTBchCode']);
            $this->db->where("ISNULL(FTShpCode,'')", $paData['FTShpCode']);
    ...
    public function FSaMCCSListDataCustomer($paData)
    {
      ...
      $nLngID    = $paData['FNLngID'];
      $tSQL = "SELECT TLKMCstBch.*,TCNMBranch_L.FTBchName FROM TLKMCstBch INNER JOIN  TCNMBranch_L  ON TLKMCstBch.FTBchCode = TCNMBranch_L.FTBchCode AND TCNMBranch_L.FNLngID ='$nLngID'";
      $oQuery = $this->db->query($tSQL);
    ...
    public function FSaMCCSListDataCustomerByid($paData)
    {
      ...
      $nLngID    = $paData['FNLngID'];
      $tSQL = "SELECT TLKMCstBch.*,TCNMBranch_L.FTBchName
      FROM TLKMCstBch
      INNER JOIN  TCNMBranch_L  ON TLKMCstBch.FTBchCode = TCNMBranch_L.FTBchCode AND TCNMBranch_L.FNLngID ='$nLngID'
      WHERE TLKMCstBch.FTBchCode = '$tBchCode' AND TLKMCstBch.FTCbrSoldTo ='$tCbrSoldTo'";
      $oQuery = $this->db->query($tSQL);
