...
function JSvDSHSALPageDashBoardMain() {
    ...
    const tAlwDashBoard = $('#ocmDSHSALAllows').val();
    $.ajax({
        type: "POST",
        ...
        success: function(tResult) {
            ...
            // Report MTD
            JSvDSHSALCallViewCompareSaleByPdtQty();
            JSvDSHSALCallViewCompareSaleByPdtAmt();
...
function JCNxDSHSALConfirmFilter(ptFilterKey) {
    ...
    $.ajax({
        ...
        data: $('#odvDSHSALModalFilter #ofmDSHSALFormFilter').serialize() + "&ptDateDataForm=" + tDateDataForm + "&ptDateDataTo=" + tDateDataTo + "&ptAlwDashBoard=" + tAlwDashBoard,
        cache: false,
        timeout: 0,
        success: function(paDataReturn) {
            ...
            setTimeout(function() {
                switch (aDataReturn['rtDSHSALTypeKey']) {
                    ...
                    case 'RPTComSalePdt':
                        {
                            // รายงานเปรียบเทียบยอดขายตามสินค้า
                            JSvDSHSALCallViewCompareSaleByPdtAmt();
...
function JSvDSHSALCallViewReportComPareSaleByPdtType() {
    ...
    if (nCookie[15] != 0) {
        ...
        // $.ajax({
        //     type: "POST",
        //     url: "dashboardsaleReportCompareSaleByPdtType",
        //     cache: false,
        //     timeout: 0,
        //     success: function(ptResult) {
        //         $('#odvDSHSALPanelReportComPareSaleByPdtType .xWDSHSALDataPanel').html(
        //             "<iframe src=\"" + $("#ohdBaseURL").val() + "dashboardsaleReportCompareSaleByPdtType\" style=\"width:100%;height:25vw;\"></iframe>"
        //         );
        //     },
        //     error: function(xhr, status, error) {
        //         JCNxResponseError(jqXHR, textStatus, errorThrown);
...
function JSvDSHSALCallViewReportComPareSaleByPdtTypeQTY() {
    ...
    if (nCookie[15] != 0) {
        ...
        // $.ajax({
        //     type: "POST",
        //     url: "dashboardsaleReportCompareSaleByPdtTypeQTY",
        //     cache: false,
        //     timeout: 0,
        //     success: function(ptResult) {
        //         $('#odvDSHSALPanelReportComPareSaleByPdtTypeQTY .xWDSHSALDataPanel').html(
        //             "<iframe src=\"" + $("#ohdBaseURL").val() + "dashboardsaleReportCompareSaleByPdtTypeQTY\" style=\"width:100%;height:25vw;\"></iframe>"
        //         );
        //     },
        //     error: function(xhr, status, error) {
        //         JCNxResponseError(jqXHR, textStatus, errorThrown);
...
// Return: View Page Main
// ReturnType: View
function JSvDSHSALCallViewCompareSaleByPdtQty() {
    const oetDSHSALUserCode = $('#oetDSHSALUserCode').val();
    const nCookie = JSvDSHSALGetCookieJS("Cookie_SKC" + oetDSHSALUserCode);
    ...
    if (nCookie[17] != 0) {
        $.ajax({
            type: "POST",
            url: "dashboardRptCompareSaleByPdtQty",
            cache: false,
            timeout: 0,
            success: function(ptResult) {

                $('#odvDSHSALPanelLeft8 .xWDSHSALDataPanelLeft').html(
                    "<iframe src=\"" + $("#ohdBaseURL").val() + "dashboardRptCompareSaleByPdtQty\" style=\"width:100%;height:27vw;padding-top:20px;\"></iframe>"
                )
            },
            error: function(xhr, status, error) {
                JCNxResponseError(jqXHR, textStatus, errorThrown);
...
// Return: View Page Main
// ReturnType: View
function JSvDSHSALCallViewCompareSaleByPdtAmt() {
    const oetDSHSALUserCode = $('#oetDSHSALUserCode').val();
    const nCookie = JSvDSHSALGetCookieJS("Cookie_SKC" + oetDSHSALUserCode);
    ...
    if (nCookie[17] != 0) {
        $.ajax({
            type: "POST",
            url: "dashboardRptCompareSaleByPdtAmt",
            cache: false,
            timeout: 0,
            success: function(ptResult) {
                $('#odvDSHSALPanelLeft9 .xWDSHSALDataPanel').html(
                    "<iframe src=\"" + $("#ohdBaseURL").val() + "dashboardRptCompareSaleByPdtAmt\" style=\"width:100%;height:27vw;padding-top:20px;\"></iframe>"
                )
            },
            error: function(xhr, status, error) {
                JCNxResponseError(jqXHR, textStatus, errorThrown);
