            ...
            <table id="otbCpgDataList" class="table table-striped"> <!-- เปลี่ยน -->
                <thead>
                    <tr>
                        ...
                        <th class="text-center xCNTextBold" style="width:8%;"><?= language('courier/courierman/courierman','tCPGTBChoose')?></th>
                        <?php endif; ?>
                        <th class="text-center xCNTextBold" style="width:5%;"><?= language('courier/courierman/courierman','tCPGTBImage')?></th>
                        <th class="text-center xCNTextBold" style="width:12%;"><?= language('courier/courierman/courierman','tCRMEmpCode')?></th>
                        <th class="text-center xCNTextBold" style="width:15%;"><?= language('courier/courierman/courierman','tCRMName')?></th>
                        <th class="text-center xCNTextBold" style="width:10%;"><?= language('courier/courierman/courierman','tCRMSex')?></th>
                        <th class="text-center xCNTextBold" style="width:20%;"><?= language('courier/courierman/courierman','tCRMCryName')?></th>
                        <th class="text-center xCNTextBold" style="width:15%;"><?= language('courier/courierman/courierman','tCRMTel')?></th>
                        <?php if($aAlwEventCourierGrp['tAutStaFull'] == 1 || $aAlwEventCourierGrp['tAutStaDelete'] == 1) : ?>
                ...
                <tbody>
                    <?php if($aCpgDataList['rtCode'] == 1 ):?>
                        <?php foreach($aCpgDataList['raItems'] AS $nKey => $aValue):?>
                            ...
                            <tr class="text-center xCNTextDetail2 otrCpg" id="otrCpg<?=$nKey?>" data-id="<?=$aValue['rtID']?>"   data-code="<?=$aValue['rtCryCode']?>" data-name="<?=$aValue['rtManName']?>">
                                <?php if($aAlwEventCourierGrp['tAutStaFull'] == 1 || $aAlwEventCourierGrp['tAutStaDelete'] == 1) : ?>
                                ...
                                <?php endif; ?>
                                <td class="text-center"><?=FCNtHGetImagePageList($aValue['FTImgObj'],'40px')?></td>
                                <td class="text-left"><?=$aValue['rtManCode']?></td>
                                <td class="text-left"><?=$aValue['rtManName']?></td>
                                <td class="text-left">
                                    <?php
                                        if($aValue['rtManSex']==1){
                                            echo "ชาย";
                                        }else{
                                            echo "หญิง";
                                        }
                                ...
                                <td class="text-left"><?=$aValue['rtCryName']?></td>
                                <td class="text-left"><?=$aValue['rtManTel']?></td>
                                <?php if($aAlwEventCourierGrp['tAutStaFull'] == 1 || $aAlwEventCourierGrp['tAutStaDelete'] == 1) : ?>
                                ...
                                <td>
                                    <!-- เปลี่ยน -->
                                    <img class="xCNIconTable" src="<?php echo  base_url().'/application/modules/common/assets/images/icons/delete.png'?>" onClick="JSoCpgDel('<?=$nCurrentPage?>','<?=$aValue['rtManName']?>','<?php echo $aValue['rtManCode']?>','<?php echo $aValue['rtID']?>','<?php echo $aValue['rtCryCode']?>')">
                                </td>
                                <?php endif; ?>
                                <?php if($aAlwEventCourierGrp['tAutStaFull'] == 1 || ($aAlwEventCourierGrp['tAutStaEdit'] == 1 || $aAlwEventCourierGrp['tAutStaRead'] == 1)) : ?>
                                <td>
                                    <!-- เปลี่ยน -->
                                    <img class="xCNIconTable" src="<?php echo  base_url().'/application/modules/common/assets/images/icons/edit.png'?>" onClick="JSvCallPageCpgEdit('<?php echo $aValue['rtCryCode'];?>','<?php echo $aValue['rtID'];?>','<?php echo $aValue['rtStaActive']; ?>')">
                                </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach;?>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
                ...
                <input type='hidden' id="ohdConfirmIDCodeDelete">
			</div>
...
$('ducument').ready(function(){
	...
	$('.ocbListItem').click(function(){
        ...
        var nID = $(this).parent().parent().parent().data('id');  //code
        ...
        if(aArrayConvert == '' || aArrayConvert == null){
            obj.push({"nCode": nCode, "tName": tName, "nID": nID});
