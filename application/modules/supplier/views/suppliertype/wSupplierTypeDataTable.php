    ...
    <div class="col-md-12">
        <div class="table-responsive">
            <table id="otbStyDataList" class="table table-striped"> <!-- เปลี่ยน -->
                <thead>
                    <tr>
                        <th class="text-center xCNTextBold" style="width:10%;"><?= language('supplier/suppliertype/suppliertype','tSTYChoose')?></th>
                            ...
                            <?php
                              ...
                              ?>
                                ...
                                <td class="text-center <?=$tDisableTD?>">
                                    <!-- เปลี่ยน -->
                                    <img class="xCNIconTable <?php echo $tDisableImg; ?>" src="<?php echo  base_url().'/application/modules/common/assets/images/icons/delete.png'?>" onClick="JSoStyDel('<?php echo $aValue['rtStyCode']?>','<?php echo $aValue['rtStyName']?>')">
                                </td>
                                <td class="text-center">
                                    <!-- เปลี่ยน -->
...
<div class="row">
    ...
    <!-- เปลี่ยน -->
    <div class="col-md-6">
        <div class="xWPageSty btn-toolbar pull-right"> <!-- เปลี่ยนชื่อ Class เป็นของเรื่องนั้นๆ --> 
            <?php if($nPage == 1){ $tDisabledLeft = 'disabled'; }else{ $tDisabledLeft = '-';} ?>
            <button onclick="JSvStyClickPage('previous')" class="btn btn-white btn-sm" <?php echo $tDisabledLeft ?>> <!-- เปลี่ยนชื่อ Onclick เป็นของเรื่องนั้นๆ --> 
                <i class="fa fa-chevron-left f-s-14 t-plus-1"></i>
            </button>
            <?php for($i=max($nPage-2, 1); $i<=max(0, min($aStyDataList['rnAllPage'],$nPage+2)); $i++){?> <!-- เปลี่ยนชื่อ Parameter Loop เป็นของเรื่องนั้นๆ --> 
                <?php 
                    if($nPage == $i){ 
                        $tActive = 'active'; 
            ...
            <button onclick="JSvStyClickPage('next')" class="btn btn-white btn-sm" <?php echo $tDisabledRight ?>> <!-- เปลี่ยนชื่อ Onclick เป็นของเรื่องนั้นๆ --> 
