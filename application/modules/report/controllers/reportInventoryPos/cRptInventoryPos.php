...
class cRptInventoryPos extends MX_Controller
{
    ...
    private function init()
    {
        $this->aText = [
            ...
            'tRptStaBalQtyTitle'      => language('report/report/report', 'tRptStaBalQtyTitle'),
            'tRptStaBalQty1'      => language('report/report/report', 'tRptStaBalQty1'),
            'tRptStaBalQty2'      => language('report/report/report', 'tRptStaBalQty2'),
        ...
        $this->aRptFilter = [
            ...
            'tRptStaBalQty' => !empty($this->input->post('ocmRptStaBalQty')) ? $this->input->post('ocmRptStaBalQty') : "",
    ...
    public function FSoCCallRptRenderFooterExcel()
    {
        ...
        if (!empty($this->aRptFilter['tWahCodeFrom']) && !empty($this->aRptFilter['tWahCodeTo'])) {
            ...
            $aMulltiRow[] = WriterEntityFactory::createRow($aCells);
                ...
                // Fillter ฟิวเตอร์ข้อมูล สถานะสินค้าคงคลัง 
                if (!empty($this->aRptFilter['tRptStaBalQty']) && !empty($this->aRptFilter['tRptStaBalQty'])) {
                    $aCells = [
                        WriterEntityFactory::createCell($this->aText['tRptStaBalQtyTitle'].' : '.$this->aText['tRptStaBalQty'.$this->aRptFilter['tRptStaBalQty']]),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
