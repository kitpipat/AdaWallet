...
class cReport extends MX_Controller {
    ...
    public function FCNoCRPTViewCondition() {
        try {
            ...
            $aCustomerInf['FTCgpCode'] = '';
            $aCustomerInf['FTCgpName'] = '';
            if(!empty($this->session->userdata("tSesUsrAgnCode"))){
                $aDataCgp = $this->mCustomer->FScMCSTGetInfoMemberConFigSpc($this->session->userdata("tSesUsrAgnCode"),1);
                    if($aDataCgp['rtCode']=='1'){
                            $aCustomerInf['FTCgpCode'] = $aDataCgp['raItem']['FTCfgStaUsrValue'];
                            $aCustomerInf['FTCgpName'] = $aDataCgp['raItem']['FTCgpName'];
                    }
            }

            $aDataView = array(
                'aRptFilterData' => $aRptFilterData,
                'aCustomerInf'   => $aCustomerInf,
            );
