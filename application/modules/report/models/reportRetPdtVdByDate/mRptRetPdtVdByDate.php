...
class mRptRetPdtVdByDate extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore = "{CALL SP_RPTxSalVdPdtRetTmp(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
        $aDataStore = array(
            'pnLngID' => $nLangID, 
            'pnComName' => $tComName,
            'ptRptCode' => $tRptCode,
    ...
    */
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tRptJoinFooter = " 
                ...
                FROM TRPTSalVdPdtRetTmp   WITH(NOLOCK)
                WHERE 1=1
        ...
        $tSQL = "   
            ...
            FROM (
                SELECT
                    ...
                    A.*,
                    S.FNRptGroupMember,S.FNSUMQty
                FROM TRPTSalVdPdtRetTmp A WITH(NOLOCK)
                /* Calculate Misures */
                LEFT JOIN (
                    SELECT
                        FTXihDocNo AS FTXihDocNo_SUM,
                        SUM(ISNULL(FCXidQty,0)) AS FNSUMQty,
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL = "   
            ...
            FROM TRPTSalVdPdtRetTmp TSBP WITH(NOLOCK)
            WHERE 1=1
    ...
    private function FMxMRPTSetPriorityGroup($paDataWhere)
    {
        ...
        $tSQL = "
            UPDATE TRPTSalVdPdtRetTmp
                SET TRPTSalVdPdtRetTmp.FNRowPartID = B.PartID
            FROM (
                SELECT
            ...
            WHERE 1=1
            AND TRPTSalVdPdtRetTmp.FTRptRowSeq = B.FTRptRowSeq
            AND TRPTSalVdPdtRetTmp.FTComName = '$tComName' 
            AND TRPTSalVdPdtRetTmp.FTRptCode = '$tRptCode'
            AND TRPTSalVdPdtRetTmp.FTUsrSession = '$tUsrSession'
        ";
        $this->db->query($tSQL);
    }
    ...
    public function FSnMCountRowInTemp($paParams)
    {
        ...
        $tSQL = "   
            SELECT
                TMP.FTRptCode
            FROM TRPTSalVdPdtRetTmp TMP WITH(NOLOCK)
