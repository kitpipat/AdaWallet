...
class mRptSaleBillPaymentDate extends CI_Model
{
    ...
    public function FSnMExecStoreCReport($paDataFilter)
    {
        ...
        $tCallStore = "{ CALL SP_RPTxVDPayByDateTmp(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
        $aDataStore = array(
    ...
    {
        $tSQL   = " UPDATE TRPTVDPayTmp
                    SET
                    TRPTVDPayTmp.FNRowPartID = B.PartID
                    FROM(
                        SELECT
                        ROW_NUMBER() OVER(PARTITION BY FTRcvCode ORDER BY FTRcvCode ASC) AS PartID ,
                            ...
                            FTRptCode,
                            FTUsrSession
                        FROM TRPTVDPayTmp TMP WITH(NOLOCK)
        ...
        //                 ) AS B
        //                 WHERE 1=1
        //                 AND TRPTVDPayTmp.FTRptRowSeq = B.FTRptRowSeq
        //                 AND TRPTVDPayTmp.FTXshDocNo  = B.FTXshDocNo
        //                 AND TRPTVDPayTmp.FTComName = '$ptComName'
        //                 AND TRPTVDPayTmp.FTRptCode = '$ptRptCode'
        //                 AND TRPTVDPayTmp.FTUsrSession = '$ptUsrSession' ";
        $tSQL  .= "
                        ) AS B
                        WHERE 1=1
                        AND TRPTVDPayTmp.FTRptRowSeq = B.FTRptRowSeq
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        FROM (
            SELECT
                ROW_NUMBER() OVER(ORDER BY FDXrcRefDate,FTRcvName,FDXrcRefTime ASC) AS RowID ,
                A.*,
                FCXrcNet_SubTotal,
                          ROW_NUMBER() OVER(PARTITION BY FDXrcRefDate ORDER BY FDXrcRefDate,FTRcvName,FDXrcRefTime ASC) AS PartFDXrcRefDate,
                         ROW_NUMBER() OVER(PARTITION BY FDXrcRefDate,FTRcvName ORDER BY FDXrcRefDate,FTRcvName,FDXrcRefTime ASC) AS PartFTRcvName,
		   PartFTRcvNameMAX
            FROM TRPTVDPayTmp A WITH(NOLOCK)
            LEFT JOIN (
                SELECT
                FTRcvName          AS FTRcvName_SUM,
                FDXrcRefDate          AS FDXrcRefDate_SUM,
                    SUM(FCXrcNet)    AS FCXrcNet_SubTotal,

                    COUNT(*) AS PartFTRcvNameMAX
                FROM TRPTVDPayTmp WITH(NOLOCK)
                WHERE 1=1
                AND FTComName       = '$tComName'
                AND FTRptCode       = '$tRptCode'
                AND FTUsrSession    = '$tUsrSession'
                GROUP BY FDXrcRefDate,FTRcvName
                ) AS S ON A.FDXrcRefDate = S.FDXrcRefDate_SUM   AND A.FTRcvName = S.FTRcvName_SUM
                WHERE 1 = 1
                AND FTComName       = '$tComName'
                AND FTRptCode       = '$tRptCode'
                AND FTUsrSession    = '$tUsrSession'
            ...
            LEFT JOIN (
                ...
                LEFT JOIN
                (
                    SELECT FDXrcRefDate AS FDXrcRefDate,
                           SUM(FCXrcNet) AS FCXrcNet_SubTotalDate,
                           COUNT(*) AS PartFDXrcRefDateMAX
                    FROM TRPTVDPayTmp WITH(NOLOCK)
                    WHERE 1 = 1
                    AND FTComName       = '$tComName'
                    AND FTRptCode       = '$tRptCode'
                ...
                ) D ON L.FDXrcRefDate = D.FDXrcRefDate
                 ";
