...
class mRptPdtSalePromotionPerDoc extends CI_Model {
    ...
    public function FSnMExecStoreReport($paDataFilter) {
        ...
        $tCallStore     = "{ CALL SP_RPTxPSBillByPdtPmt(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
    ...
    public function FSaMGetDataReport($paDataWhere){
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter = "   
                ...
                FROM TRPTPSBillByPdtPmtTmp WITH(NOLOCK)
                WHERE 1=1
        ...
        $tSQL   = " SELECT
                    ...
                    FROM (
                        ...
                        FROM TRPTPSBillByPdtPmtTmp A WITH(NOLOCK)
                        /* Calculate Misures */
                        LEFT JOIN (
                            SELECT
    ...
    public function FMxMRPTSetPriorityGroup($paDataWhere){
        ...
        $tSQL   = " UPDATE TRPTPSBillByPdtPmtTmp
                        SET FNRowPartID = B.PartID
                    FROM(
                        SELECT
                            ROW_NUMBER() OVER(PARTITION BY FTPmhDocNo,FTXpdGetType ORDER BY FTPmhDocNo DESC) AS PartID,
                            FTRptRowSeq
                        FROM TRPTPSBillByPdtPmtTmp TMP WITH(NOLOCK)
                        WHERE TMP.FTComName     = '$tComName' 
                        AND TMP.FTRptCode       = '$tRptCode'
                        AND TMP.FTUsrSession    = '$tUsrSession'
                    ) AS B
                    WHERE TRPTPSBillByPdtPmtTmp.FTRptRowSeq = B.FTRptRowSeq
                    AND TRPTPSBillByPdtPmtTmp.FTComName      = '$tComName'
                    AND TRPTPSBillByPdtPmtTmp.FTRptCode      = '$tRptCode'
                    AND TRPTPSBillByPdtPmtTmp.FTUsrSession   = '$tUsrSession'
                ";
