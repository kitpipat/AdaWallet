...
class mRptSalTimePrdTmp extends CI_Model
{
    //Functionality: Delete Temp Report
    //Parameters:  Function Parameter
    //Creator: 21/10/2020 Sooksanti
    //Last Modified :
    ...
    public function FSnMExecStoreReport($paData)
    {
        ...
        $tCallStore = "{ CALL SP_RPTxSalTimePrdTmp001001031(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
        $aDataStore = array(
    ...
    public function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL = "   SELECT
                    ...
                    FROM TRPTSalTimePrdTmp AS SAL WITH(NOLOCK)
                    WHERE 1=1
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter = "   SELECT
                                        ...
                                        SUM(FCXshBillQty)       AS FCXshBillQty_Footer,
                                        SUM(FCXsdNet)           AS FCXsdNet_Footer,
                                        SUM(FCXsdDisChg)        AS FCXsdDisChg_Footer,
                                        SUM(FCXshGrand)         AS FCXshGrand_Footer
                                    FROM TRPTSalTimePrdTmp WITH(NOLOCK)
                                    WHERE 1=1
        ...
        $tSQL = "   SELECT
                        ...
                        FROM (
                            SELECT
                                ...
                                S.FNRptGroupMember,
                                S.FCXsdQty_SubTotal,
                                S.FCXshBillQty_SubTotal,
                                S.FCXsdNet_SubTotal,
                                S.FCXsdDisChg_SubTotal,
                                S.FCXshGrand_SubTotal
                            FROM TRPTSalTimePrdTmp A WITH(NOLOCK)
                            /* Calculate Misures */
                            LEFT JOIN (
                                SELECT
                                    ...
                                    SUM(FCXshBillQty)       AS FCXshBillQty_SubTotal,
                                    SUM(FCXsdNet)           AS FCXsdNet_SubTotal,
                                    SUM(FCXsdDisChg)        AS FCXsdDisChg_SubTotal,
