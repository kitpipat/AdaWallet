...
class mRptSalByDT extends CI_Model {
    ...
    public function FSnMExecStoreReport($paDataFilter) {
        ...
        $tCallStore = "{ CALL SP_RPTxSpcSalByDT(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
        $aDataStore = array(
            'pnLngID'      => $nLangID,
            'pnComName'    => $tComName,
            'ptRptCode'    => $tRptCode,
    ...
    public function FSaMGetDataReport($paDataWhere){
        ...
        if($nPage == $nTotalPage){
            $tRptJoinFooter = " SELECT
                                    ...
                                    CONVERT(FLOAT,SUM(FCXsdSetPrice))	AS FCXsdSetPrice_Footer,
                                    CONVERT(FLOAT,SUM(FCXsdGrossSales))    AS FCXsdGrossSales_Footer,
                                    CONVERT(FLOAT,SUM(FCXsdGrossSalesExVat))	AS FCXsdGrossSalesExVat_Footer,
                                    CONVERT(FLOAT,SUM(FCXsdNetSales))	AS FCXsdNetSales_Footer,
                                    CONVERT(FLOAT,SUM(FCXsdNetSalesEx))    AS FCXsdNetSalesEx_Footer,
                                    CONVERT(FLOAT,SUM(FCXsdDis))	AS FCXsdDis_Footer
                                FROM TRPTSpcSalByDTTmp WITH(NOLOCK)
                                WHERE 1=1
            ...
            $tRptJoinFooter = " 
                SELECT
                    ...
                    0 AS FCXsdGrossSales_Footer,
                    0 AS FCXsdGrossSalesExVat_Footer,
                    0 AS FCXsdNetSales_Footer,
                    0 AS FCXsdNetSalesEx_Footer,
        ...
        $tSQL   =   "   SELECT
                        ...
                        FROM (
                            SELECT
                                ROW_NUMBER() OVER(ORDER BY DATA.FDXshDocDate ASC) AS RowID,
                                DATA.*
                            FROM TRPTSpcSalByDTTmp DATA WITH(NOLOCK)
                            WHERE 1=1
    ...
    public function FMxMRPTSetPriorityGroup($paDataWhere){
        ...
        $tSQL = " 
                UPDATE TRPTSpcSalByDTTmp
                SET TRPTSpcSalByDTTmp.FNRowPartID = B.PartID
                FROM (
                    SELECT
                        ROW_NUMBER() OVER(PARTITION BY TSPT.FDXshDocDate ORDER BY TSPT.FDXshDocDate ASC) AS PartID ,
                        TSPT.FTRptRowSeq
                    FROM TRPTSpcSalByDTTmp TSPT WITH(NOLOCK)
            ...
            WHERE 1=1
            AND TRPTSpcSalByDTTmp.FTRptRowSeq = B.FTRptRowSeq
            AND TRPTSpcSalByDTTmp.FTComName = '$tComName' 
            AND TRPTSpcSalByDTTmp.FTRptCode = '$tRptCode'
            AND TRPTSpcSalByDTTmp.FTUsrSession = '$tUsrSession' ";
    ...
    public function FSaMGetDataSumFootReport($paDataWhere){
        ...
        $tSQL = "
            SELECT 
                ISNULL(SUM(FCXsdSalePrice),0) AS FCXsdSalePriceSum,
                ISNULL(SUM(FCXsdGrossSales),0) AS FCXsdGrossSalesSum,
                ISNULL(SUM(FCXsdGrossSalesExVat),0) AS FCXsdGrossSalesExVatSum
            FROM TRPTSpcSalByDTTmp
