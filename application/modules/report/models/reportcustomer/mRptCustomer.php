...
class mRptCustomer extends CI_Model {
    ...
    public function FSnMExecStoreReport($paDataFilter) {
        ...
        $tCallStore = "{ CALL SP_RPTxCstSalMTD(?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
        $aDataStore = array(
            'pnLngID'      => $nLangID,
            'pnComName'    => $tComName,
            'ptRptCode'    => $tRptCode,
            ...
            'tSpcAgncyCode' => $paDataFilter['tSpcAgncyCode'],
            'ptCstCodeF'   => $paDataFilter['tCstCodeFrom'],
            'ptCstCodeT'   => $paDataFilter['tCstCodeTo'],
            'tRptCgpCodeFrm' => $paDataFilter['tRptCgpCodeFrm'],
            'tRptCtyCodeFrm' => $paDataFilter['tRptCtyCodeFrm'],
            'tRptCtyCodeTo' => $paDataFilter['tRptCtyCodeTo'],
            'tRptClvCodeFrm' => $paDataFilter['tRptClvCodeFrm'],
            'tRptClvCodeTo' => $paDataFilter['tRptClvCodeTo'],
            'FNResult'     => 0,
        );
    ...
    public function FSaMGetDataReport($paDataWhere){
        ...
        if($nPage == $nTotalPage){
            $tRptJoinFooter = " SELECT
                                    ...
                                    CONVERT(FLOAT,SUM(FCTxnBuyTotal))	AS FCTxnBuyTotal_Footer,
                                    CONVERT(FLOAT,SUM(FCXshGrand))    AS FCXshGrand_Footer,
                                    CONVERT(FLOAT,SUM(FCTxnPntQtyBal))	AS FCTxnPntQtyBal_Footer
                                FROM TRPTCstSalMTDTmp WITH(NOLOCK)
                                WHERE 1=1
            ...
            $tRptJoinFooter = " 
                SELECT
                    ...
                    0 AS FCTxnBuyTotal_Footer,
                    0 AS FCXshGrand_Footer,
                    0 AS FCTxnPntQtyBal_Footer
                ) T ON  L.FTUsrSession = T.FTUsrSession_Footer
            ";
        }
        ...
        $tSQL   =   "   SELECT
                        ...
                        FROM (
                            SELECT
                                ROW_NUMBER() OVER(ORDER BY DATA.FTCstCode ASC) AS RowID,
                                DATA.*
                            FROM TRPTCstSalMTDTmp DATA WITH(NOLOCK)
                            WHERE 1=1
    ...
    public function FMxMRPTSetPriorityGroup($paDataWhere){
        ...
        $tSQL = " 
                UPDATE TRPTCstSalMTDTmp
                SET TRPTCstSalMTDTmp.FNRowPartID = B.PartID
                FROM (
                    SELECT
                        ROW_NUMBER() OVER(PARTITION BY TSPT.FTCstCode ORDER BY TSPT.FTCstCode ASC) AS PartID ,
                        TSPT.FTRptRowSeq
                    FROM TRPTCstSalMTDTmp TSPT WITH(NOLOCK)
            ...
            WHERE 1=1
            AND TRPTCstSalMTDTmp.FTRptRowSeq = B.FTRptRowSeq
            AND TRPTCstSalMTDTmp.FTComName = '$tComName' 
            AND TRPTCstSalMTDTmp.FTRptCode = '$tRptCode'
            AND TRPTCstSalMTDTmp.FTUsrSession = '$tUsrSession' ";
