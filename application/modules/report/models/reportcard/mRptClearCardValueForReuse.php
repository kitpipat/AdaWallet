...
class mRptClearCardValueForReuse extends CI_Model {
    ...
    public function FSnMExecStoreReport($paDataFilter = []) {
        ...
        // ตัวแทนขาย
        $tAgnCodeSelect  = $paDataFilter['tAgnCodeSelect'];
    ...
    public function FSaMGetDataReport($paDataWhere = [], $paDataFilter = []) {
        ...
        $tSQL  = "  
            SELECT
                ...
                FROM(
                    SELECT
                        ROW_NUMBER() OVER(ORDER BY TMP.FTCrdCode ASC) AS rtRowID, TMP.* 
                    FROM TFCTRptCrdValTmp TMP WITH (NOLOCK)
                    WHERE 1=1 AND TMP.FTComName = '$tCompName' AND TMP.FTRptCode = '$tRptCode'
                    /*AND FTTxnDocType = 10*/   
                ) AS RPT        
            WHERE RPT.rtRowID > $aRowLen[0] AND RPT.rtRowID <= $aRowLen[1]
            ORDER BY RPT.FTCrdCode ASC
        ";
    ...
    public function FSaMCountDataReportAll($paDataWhere) {
        ...
        $tSQL = "   
            SELECT 
                FTComName
            FROM TFCTRptCrdValTmp WITH (NOLOCK)
