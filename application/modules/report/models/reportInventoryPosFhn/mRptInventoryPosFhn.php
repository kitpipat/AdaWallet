...
class mRptInventoryPosFhn extends CI_Model {
    ...
    public function FSnMExecStoreCReport($paDataFilter) {
        ...
        $tCallStore = "{ CALL SP_RPTxSpcStockBalFhn(?,?,?,?,?,?,?,?,?,?,?) }";
        $aDataStore = array(
        ...
        $tSQL = "
		    ...
		    FROM( 
                SELECT 
                    ...
                    FTRptRowSeq  
                FROM TRPTPdtStkBalFhnTmp  TMP WITH(NOLOCK)
                WHERE TMP.FTComName = '$ptComName' 
                AND TMP.FTRptCode = '$ptRptCode'
                AND TMP.FTUsrSession = '$ptUsrSession'
            ) B
            WHERE TRPTPdtStkBalFhnTmp .FTRptRowSeq = B.FTRptRowSeq 
            AND TRPTPdtStkBalFhnTmp .FTComName     = '$ptComName' 
            AND TRPTPdtStkBalFhnTmp .FTRptCode     = '$ptRptCode'
            AND TRPTPdtStkBalFhnTmp .FTUsrSession  = '$ptUsrSession' 
        ";
    ...
    public function FMaMRPTPagination($paDataWhere) {
        ...
        $tSQL = "   
            SELECT
                TT_TMP.FTWahCode
            FROM TRPTPdtStkBalFhnTmp  TT_TMP WITH(NOLOCK)
    ...
    public function FSaMGetDataReport($paDataWhere) {
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter = "   
                SELECT 
                    ...
                    SUM(FCXshNetSale)  AS FCXshNetSale_Footer,
                    SUM(FCXshDiffCost)  AS FCXshDiffCost_Footer
                FROM TRPTPdtStkBalFhnTmp  WITH(NOLOCK)
                WHERE 1=1
            ...
            $tJoinFoooter = "   
                SELECT
                    ...
                    0               AS FCXshNetSale_Footer,
                    0               AS FCXshDiffCost_Footer
        ...
        $tSQL = "   
            SELECT
                ...
                T.FCXshNetSale_Footer,
                T.FCXshDiffCost_Footer,
                T.FCStkQty_Footer
            FROM (
                SELECT  
                    ...
                    S.FNRptGroupMember,
                    S.FCStkQty_SubTotal
                FROM TRPTPdtStkBalFhnTmp  A WITH(NOLOCK)
            ...
            LEFT JOIN (
                SELECT
                    ...
                    SUM(FCXshNetSale)   AS FCXshNetSale_SUM,
                    SUM(FCXshDiffCost)   AS FCXshDiffCost_SUM
                FROM TRPTPdtStkBalFhnTmp  WITH(NOLOCK)
                WHERE 1=1
                AND FTComName       = '$tComName'
                AND FTRptCode       = '$tRptCode'
