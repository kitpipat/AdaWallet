...
class mRptInventory extends CI_Model {
    ...
    public function FMxMRPTSetPriorityGroup($ptComName, $ptRptCode, $ptUsrSession) {
        $tSQL   = " UPDATE TRPTVDPdtStkBalTmp 
                    ...
                    FROM(
                        SELECT
                            ROW_NUMBER() OVER(PARTITION BY FTBchCode,FTPosCode ORDER BY FTBchCode,FTPosCode DESC) AS PartID , 
                            FTRptRowSeq
    ...
    public function FSaMGetDataReport($paDataWhere) {
        ...
        $tSQL = "   
            ...
            FROM (
                SELECT  
                    ROW_NUMBER() OVER(ORDER BY FTBchCode,FTPosCode) AS RowID ,
                    A.*,
                    S.FNRptGroupMember,
                ...
                LEFT JOIN (
                    ...
                    AND FTRptCode       = '$tRptCode'
                    AND FTUsrSession    = '$tUsrSession'
                    GROUP BY FTPosCode,FTBchCode
                ) AS S ON A.FTPosCode = S.FTPosCode_SUM AND A.FTBchCode = S.FTBchCode_SUM
