...
class mRptMovePosVdFhn extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore = "{ CALL SP_RPTxSpcStockMoventFhn(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
        $aDataStore = array(
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter   =   "   SELECT
                                    ...
                                    FROM TRPTPdtStkCrdFhnTmp WITH(NOLOCK)
                                    WHERE 1=1
        ...
        $tSQL   = " SELECT
                    ...
                    FROM (
                        ...
                        ROW_NUMBER() OVER(ORDER BY DATASTKCRD.FTBchCode ASC,DATASTKCRD.FTWahCode ASC,DATASTKCRD.FTPdtCode ASC,DATASTKCRD.FDStkDate ASC, DATASTKCRD.FTRptRowSeq ASC) AS RowID,
                            ...
                            DATASUBWAH.*,
                            DATASUBBCH.*
                        FROM TRPTPdtStkCrdFhnTmp DATASTKCRD WITH(NOLOCK)
                        LEFT JOIN (
                            SELECT DISTINCT
    ...
    public function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL           =   "   SELECT
                                ...
                                FROM TRPTPdtStkCrdFhnTmp STK WITH(NOLOCK)
                                WHERE 1=1
    ...
    public function FMxMRPTSetPriorityGroup($ptComName, $ptRptCode, $ptUsrSession)
    {
        $tSQLUPD   = "  UPDATE DATAUPD
                        ...
                        FROM TRPTPdtStkCrdFhnTmp DATAUPD
                        RIGHT JOIN (
                            SELECT 
    ...
    public function FSnMCountDataReportAll($paDataWhere)
    {
        ...
        $tSQL = "   
            SELECT 
                DTTMP.FTRptCode
            FROM TRPTPdtStkCrdFhnTmp AS DTTMP WITH(NOLOCK)
