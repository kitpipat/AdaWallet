...
//ฟังก์ชันกดว่าข้อความอ่านแล้ว
function JSxCNEventChangeDocBranch(paData){
    JCNxOpenLoading();
    $.ajax({
        type    : "POST",
        url     : "docCNUpdBchEntChg",
        data    : {
                    'rtDocBchCode':paData.rtDocBchCode,
                    'rtDocNo':paData.rtDocNo,
                    'rtDocKey':paData.rtDocKey
                    },
        timeout: 0,
        cache   : false,
        success: function (tResult){
            ...
            if(aData['nStaEvent']!='1'){
                FSvCMNSetMsgErrorDialog(aData['tStaMessg']);
            }
            JCNxCloseLoading();
        },
        error: function (data) {
